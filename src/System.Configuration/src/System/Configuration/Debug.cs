...
namespace System.Configuration
{
    internal static class Debug
    {
        ...
        internal const string TagAll = "*";

        internal const string DateFormat = @"yyyy/MM/dd HH:mm:ss.ffff";
        internal const string TimeFormat = @"HH:mm:ss:ffff";
        ...
        private static class NativeMethods
        {
            ...
            [ResourceExposure(ResourceScope.None)]
            internal static extern int MessageBox(HandleRef hWnd, string text, string caption, int type);


            [DllImport("advapi32.dll", CharSet = CharSet.Auto, BestFitMapping = false, SetLastError = true)]
            [ResourceExposure(ResourceScope.Machine)]
            internal static extern int RegOpenKeyEx(IntPtr hKey, string lpSubKey, int ulOptions, int samDesired,
                out SafeRegistryHandle hkResult);

            [DllImport("advapi32.dll", ExactSpelling = true, SetLastError = true)]
            [ResourceExposure(ResourceScope.None)]
            internal static extern int RegNotifyChangeKeyValue(SafeRegistryHandle hKey, bool watchSubTree,
                uint notifyFilter, SafeWaitHandle regEvent, bool async);
        ...
        private class Tag
        {
            ...
            internal Tag(string name, TagValue value)
            {
                ...
                if (_name[_name.Length - 1] == '*') _prefixLength = _name.Length - 1;
                else _prefixLength = -1;
            }
            ...
            if (!s_inited)
            {
                lock (s_lock)
                {
                    if (!s_inited)
                    {
                        ...
                        tagDefaults.Add(new Tag(TagAll, TagValue.Disabled));
                        tagDefaults.Add(new Tag(TagInternal, TagValue.Enabled));
                        tagDefaults.Add(new Tag(TagExternal, TagValue.Enabled));
        ...
        private static void WriteTagsToRegistry()
        {
            try
            {
                using (RegistryKey key = Registry.LocalMachine.CreateSubKey(s_regKeyName))
                {
                    ...
                    foreach (Tag tag in tags) key.SetValue(tag.Name, tag.Value, RegistryValueKind.DWord);
                }
            }
            catch { }
        }
        ...
        private static void ReadTagsFromRegistry()
        {
            lock (s_lock)
            {
                try
                {
                        ...
                        if (key != null)
                        {
                            ...
                            foreach (string name in names)
                            {
                                ...
                                try
                                {
                                    ...
                                    if ((TagValue.Min <= keyvalue) && (keyvalue <= TagValue.Max)) value = keyvalue;
                                    else writeTags = true;
                                }
                                catch
                                {
                    ...
                    if (writeTags || ((names != null) && (names.Length < tags.Count))) WriteTagsToRegistry();
                }
                catch
                {
                    s_tags = new List<Tag>(s_tagDefaults);
        ...
        public static void OnRegChangeKeyValue(object state, bool timedOut)
        {
            if (!s_stopMonitoring)
            {
                if (timedOut) StopRegistryMonitor();
                else
                {
                    // Monitor again
                    MonitorRegistryForOneChange();
        ...
        private static void MonitorRegistryForOneChange()
        {
            ...
            int result = NativeMethods.RegOpenKeyEx(NativeMethods.HKEY_LOCAL_MACHINE, s_listenKeyName, 0,
                NativeMethods.KEY_READ, out regHandle);
            ...
            if (result != 0) StopRegistryMonitor();
        }
        ...
        private static Tag FindMatchingTag(string name, bool exact)
        {
            ...
            foreach (Tag tag in tags) if (StringEqualsIgnoreCase(name, tag.Name)) return tag;
        ...
        private static bool TraceBreak(string tagName, string message, Exception e, bool includePrefix)
        {
            ...
            if (tagValue == TagValue.Disabled) return false;

            bool isAssert = ReferenceEquals(tagName, TAG_ASSERT);
            if (isAssert) tagName = "";
            ...
            if (e != null)
            {
                ...
                if (errorCode != null) exceptionMessage = "Exception " + e.ToString() + "\n" + errorCode;
                else exceptionMessage = "Exception " + e.ToString();
            }
            ...
            if (!includePrefix || !s_includePrefix) traceFormat = "{4}\n{5}";
            else
            {
...
#pragma warning restore 0618
                ...
                else traceFormat = "[{2} {3}] {4}\n{5}";
            }

            string suffix = "";
            if (exceptionMessage != null) suffix += exceptionMessage + "\n";

            bool doBreak = tagValue == TagValue.Break;
            if (doBreak && !isAssert) suffix += "Breaking into debugger...\n";

            string traceMessage = string.Format(CultureInfo.InvariantCulture, traceFormat, idProcess, idThread,
                COMPONENT, tagName, message, suffix);
        ...
        private static bool DoAssert(string message)
        {
            if (!s_assert) return false;

            // Skip 2 frames - one for this function, one for
            // the public Assert function that called this function.
            Diagnostics.StackFrame frame = new Diagnostics.StackFrame(2, true);
            Diagnostics.StackTrace trace = new Diagnostics.StackTrace(2, true);
            ...
            else traceFormat = "ASSERTION FAILED: {0}\nStack trace:\n{3}";

            string traceMessage = string.Format(CultureInfo.InvariantCulture, traceFormat, message, fileName, lineNumber,
                trace.ToString());
...
#pragma warning restore 0618
            ...
            Thread thread = new Thread(
                delegate ()
                {
                    for (int i = 0; i < 100; i++)
                    {
                        ...
                        NativeMethods.PeekMessage(ref msg, new HandleRef(mbResult, IntPtr.Zero), 0, 0,
                            NativeMethods.PM_REMOVE);
                    }

                    mbResult.Result = NativeMethods.MessageBox(new HandleRef(mbResult, IntPtr.Zero), dialogMessage,
                        PRODUCT + " Assertion",
        ...
        {
#if DEBUG
            if (TraceBreak(tagName, message, null, true)) Break();
#endif
        }
        ...
        {
#if DEBUG
            if (TraceBreak(tagName, message, null, includePrefix)) Break();
#endif
        }
        ...
        {
#if DEBUG
            if (TraceBreak(tagName, message, e, true)) Break();
#endif
        }
        ...
        {
#if DEBUG
            if (TraceBreak(tagName, null, e, true)) Break();
#endif
        }
        ...
        {
#if DEBUG
            if (TraceBreak(tagName, message, e, includePrefix)) Break();
#endif
        }
...
#if DEBUG
            EnsureInit();
            if (assertion == false) if (DoAssert(message)) Break();
#endif
        }
...
#if DEBUG
            EnsureInit();
            if (assertion == false) if (DoAssert(null)) Break();
#endif
        }
        ...
        internal static string GetDescription(Object obj, string indent)
        {
            ...
            else
            {
                ...
                if ((mi == null) || (mi.ReturnType != typeof(string))) description = indent + obj.ToString();
                else
                {
                    parameters = new Object[1] { indent };
        ...
        internal static void Dump(string tagName, object obj)
        {
            ...
            if (obj != null)
            {
                ...
                {
                    if (IsTagEnabled(tagName)) traceTag = tagName;
                    else
                    {
                        if (dumpEnabled) traceTag = "Dump";
                    }
