...
namespace System.Composition.TypedParts.Discovery
{
    ...
    class DiscoveredPart
    {
        ...
        private static readonly IDictionary<string, object> _NoMetadata = new Dictionary<string, object>();
        private static readonly MethodInfo _ActivatorInvoke = typeof(CompositeActivator).GetTypeInfo().GetDeclaredMethod("Invoke");
            ...
            for (var i = 0; i < cps.Length; ++i)
            {
                ...
                if (partActivatorDependencies.TryGetValue(pi, out dep))
                {
                    ...
                    paramActivatorCalls[i] =
                        Expression.Convert(Expression.Call(Expression.Constant(a), _ActivatorInvoke, contextParam, operationParm), pi.ParameterType);
                }
                else
                {
        ...
        public IDictionary<string, object> GetPartMetadata(TypeInfo partType)
        {
            ...
            return partMetadata.Count == 0 ? _NoMetadata : partMetadata;
        }
