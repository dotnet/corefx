...
namespace System.Xml.XPath
{
    ...
    internal class XPathNavigatorReader : XmlReader, IXmlNamespaceResolver
    {
        ...
        protected XPathNavigatorReader(XPathNavigator navToRead)
        {
            ...
            this._navToRead = navToRead;
            this._nav = XmlEmptyNavigator.Singleton;
            this._state = State.Initial;
            this._depth = 0;
            this._nodeType = XPathNavigatorReader.ToXmlNodeType(this._nav.NodeType);
        }
        ...
        public override XmlNameTable NameTable
        {
            get
            {
                return this._navToRead.NameTable;
            }
        }

        IDictionary<string, string> IXmlNamespaceResolver.GetNamespacesInScope(XmlNamespaceScope scope)
        {
            return this._nav.GetNamespacesInScope(scope);
        }

        string IXmlNamespaceResolver.LookupNamespace(string prefix)
        {
            return this._nav.LookupNamespace(prefix);
        }

        string IXmlNamespaceResolver.LookupPrefix(string namespaceName)
        {
            return this._nav.LookupPrefix(namespaceName);
        }
        ...
        {
            get { return this._nav.ValueType; }
        }
        ...
        public override string LocalName
        {
            get
            {
                ...
                if (this._nav.NodeType == XPathNodeType.Namespace && this._nav.LocalName.Length == 0)
                    return this.NameTable.Add("xmlns");
                //Special case attribute text node
                if (this.NodeType == XmlNodeType.Text)
                    return string.Empty;
                return this._nav.LocalName;
            }
        }
        ...
        public override string BaseURI
        {
            get
            {
                ...
                if (this._state == State.Initial)
                    return this._navToRead.BaseURI;
                return this._nav.BaseURI;
            }
        }
        ...
        public override bool IsEmptyElement
        {
            get
            {
                return this._nav.IsEmptyElement;
            }
        }
        ...
        public override XmlSpace XmlSpace
        {
            get
            {
                XPathNavigator tempNav = this._nav.Clone();
                do
                {
        ...
        public override bool HasValue
        {
            get
            {
                if ((this._nodeType != XmlNodeType.Element)
                    && (this._nodeType != XmlNodeType.Document)
        ...
        private XPathNavigator GetElemNav()
        {
            ...
            {
                case State.Content:
                    return this._nav.Clone();
                case State.Attribute:
                case State.AttrVal:
                    tempNav = this._nav.Clone();
        ...
        {
            this._nav.MoveTo(nav);
            this._depth = depth;
            this._nodeType = XmlNodeType.Attribute;
            this._state = State.Attribute;
        }
        ...
        public override string GetAttribute(string name)
        {
            ...
            if (0 == prefix.Length)
            {
                ...
                if ((object)nav == (object)this._nav)
                    nav = nav.Clone();
        ...
        public override bool MoveToNextAttribute()
        {
            switch (this._state)
            {
                ...
                case State.Attribute:
                    {
                        if (XPathNodeType.Attribute == this._nav.NodeType)
                            return this._nav.MoveToNextAttribute();

                        // otherwise it is on a namespace... namespace are in reverse order
                        Debug.Assert(XPathNodeType.Namespace == this._nav.NodeType);
                        XPathNavigator nav = this._nav.Clone();
                        if (!nav.MoveToParent())
                            return false; // shouldn't happen
                        if (!nav.MoveToFirstNamespace(XPathNamespaceScope.Local))
                            return false; // shouldn't happen
                        if (nav.IsSamePosition(this._nav))
                        {
        ...
        public override bool ReadAttributeValue()
        {
            ...
            if (this._state == State.Attribute)
            {
                this._state = State.AttrVal;
        ...
        public override bool Read()
        {
            ...
            switch (this._state)
            {
                ...
                case State.Initial:
                    ...
                    this._nav = this._navToRead;
                    this._state = State.Content;
                    if (XPathNodeType.Root == this._nav.NodeType)
                    {
                        ...
                        this._readEntireDocument = true;
                    }
                    else if (XPathNodeType.Attribute == this._nav.NodeType)
                    {
                        this._state = State.Attribute;
                    }
                    this._nodeType = ToXmlNodeType(this._nav.NodeType);
                    break;
                ...
                case State.Content:
                    ...
                    else if (this._nodeType == XmlNodeType.Element
                        && !this._nav.IsEmptyElement)
                    {
                        this._nodeType = XmlNodeType.EndElement;
                        this._state = State.EndElement;
                    }
                    else
                ...
                case State.EndElement:
                    if (0 == _depth && !this._readEntireDocument)
                    {
                        SetEOF();
                        return false;
                    }
                    ...
                    else if (_depth > 0 && this._nav.MoveToParent())
                    {
                        Debug.Assert(this._nav.NodeType == XPathNodeType.Element, this._nav.NodeType.ToString() + " == XPathNodeType.Element");
                        this._nodeType = XmlNodeType.EndElement;
                        this._state = State.EndElement;
                        _depth--;
                    }
    ...
    {
        private static volatile XmlEmptyNavigator _singleton;
        ...
        public static XmlEmptyNavigator Singleton
        {
            get
            {
                if (XmlEmptyNavigator._singleton == null)
                    XmlEmptyNavigator._singleton = new XmlEmptyNavigator();
                return XmlEmptyNavigator._singleton;
            }
        }
