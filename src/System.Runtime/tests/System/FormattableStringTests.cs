...
namespace System.Tests
{
    public class FormattableStringTests : RemoteExecutorTestBase
    {
        ...
        [Fact]
        public static void CurrentCulture_DutchCulture_ThrowsArgumentNullException()
        {
            RemoteInvoke(
                () =>
                {
                    ...
                    double d = 123.456;
                    string text = FormattableString.CurrentCulture($"Dutch decimal separator is comma {d}");
                    Assert.Equal("Dutch decimal separator is comma 123,456", text);
