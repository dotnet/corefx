...
namespace System.Runtime.Tests
{
    public static class GuidTests
    {
        ...
        public static void TestParse(string input, string format, Guid expected)
        {
            ...
            Assert.Equal(expected, Guid.ParseExact(input, format.ToLowerInvariant())); // Format is case insensitive
            ...
            Guid result3;
            Assert.True(Guid.TryParseExact(input, format.ToLowerInvariant(), out result3)); // Format is case insensitive
        ...
        [Theory, MemberData("GuidStrings_Format_Invalid_TestData")]
        ...
        [Theory, MemberData("CompareToTestData")]
        public static void TestCompareTo(Guid guid1, object obj, int expected)
        {
            if (obj is Guid)
            {
                Assert.Equal(expected, CompareHelper.NormalizeCompare(guid1.CompareTo((Guid)obj)));
            }
