...
namespace System.Runtime.Tests
{
    public static class StringTests
    {
        ...
        public static void TestCompare(string strA, int indexA, string strB, int indexB, int length, StringComparison comparisonType, int expected)
        {
            ...
            if (hasNullInputs || indexesReferToEntireString)
            {
                if (comparisonType == StringComparison.CurrentCulture)
                {
                    ...
                    Assert.Equal(expected, NormalizeCompare(string.Compare(strA, strB, false)));
                    if (strA != null)
                    {
                        Assert.Equal(expected, NormalizeCompare(strA.CompareTo(strB)));

                        IComparable iComparable = strA;
                        Assert.Equal(expected, NormalizeCompare(iComparable.CompareTo(strB)));
                    }
                }
                else if (comparisonType == StringComparison.CurrentCultureIgnoreCase)
                {
                    Assert.Equal(expected, NormalizeCompare(string.Compare(strA, strB, true)));
                }
        ...
        {
            Assert.Throws<ArgumentException>("comparisonType", () => string.Compare("a", "bb", StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => string.Compare("a", "bb", StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type

            Assert.Throws<ArgumentException>("comparisonType", () => string.Compare("a", 0, "bb", 0, 1, StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => string.Compare("a", 0, "bb", 0, 1, StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type

            Assert.Throws<ArgumentOutOfRangeException>("offset1", () => string.Compare("a", -1, "bb", 0, 1)); // IndexA < 0
            Assert.Throws<ArgumentOutOfRangeException>("indexA", () => string.Compare("a", -1, "bb", 0, 1, StringComparison.CurrentCulture)); // IndexA < 0
            Assert.Throws<ArgumentOutOfRangeException>("length1", () => string.Compare("a", 2, "bb", 0, 1)); // IndexA > stringA.Length
            Assert.Throws<ArgumentOutOfRangeException>("indexA", () => string.Compare("a", 2, "bb", 0, 1, StringComparison.CurrentCulture)); // IndexA > stringA.Length

            Assert.Throws<ArgumentOutOfRangeException>("offset2", () => string.Compare("a", 0, "bb", -1, 1)); // IndexB < 0
            Assert.Throws<ArgumentOutOfRangeException>("indexB", () => string.Compare("a", 0, "bb", -1, 1, StringComparison.CurrentCulture)); // IndexB < 0
            Assert.Throws<ArgumentOutOfRangeException>("length2", () => string.Compare("a", 0, "bb", 3, 0)); // IndexB > stringB.Length
            Assert.Throws<ArgumentOutOfRangeException>("indexB", () => string.Compare("a", 0, "bb", 3, 0, StringComparison.CurrentCulture)); // IndexB > stringB.Length

            Assert.Throws<ArgumentOutOfRangeException>("length1", () => string.Compare("a", 0, "bb", 0, -1)); // Length < 0
            Assert.Throws<ArgumentOutOfRangeException>("length", () => string.Compare("a", 0, "bb", 0, -1, StringComparison.CurrentCulture)); // Length < 0
        }

        [Fact]
        public static void TestCompareOrdinal_Invalid()
        {
            Assert.Throws<ArgumentOutOfRangeException>("indexA", () => string.CompareOrdinal("a", -1, "bb", 0, 0)); // IndexA < 0
            Assert.Throws<ArgumentOutOfRangeException>("indexA", () => string.CompareOrdinal("a", 6, "bb", 0, 0)); // IndexA > strA.Length
            ...
            Assert.Throws<ArgumentOutOfRangeException>("count", () => string.CompareOrdinal("a", 0, "bb", 0, -1)); // Length < 0
        }
        
        public static IEnumerable<object[]> TestConcat_Strings_TestData()
        {
        ...
        [Theory, MemberData("TestConcat_Strings_TestData")]
        ...
        public static IEnumerable<object[]> TestConcat_Objects_TestData()
        {
        ...
        [Theory, MemberData("TestConcat_Objects_TestData")]
        ...
        [Fact]
        public static void TestContains_Invalid()
        {
            Assert.Throws<ArgumentNullException>("value", () => "foo".Contains(null)); // Value is null
        }
        ...
        {
            var dst = new char[expected.Length];
        ...
        public static void TestCopyTo_Invalid()
        {
            ...
            Assert.Throws<ArgumentOutOfRangeException>("sourceIndex", () => s.CopyTo(s.Length, dst, 0, 1)); // Source index + count > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("sourceIndex", () => s.CopyTo(s.Length - 1, dst, 0, 2)); // Source index + count > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("sourceIndex", () => s.CopyTo(0, dst, 0, 6)); // Source index + count > string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>("value", () => "foo".EndsWith(null)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => "foo".EndsWith(null, StringComparison.CurrentCulture)); // Value is null

            Assert.Throws<ArgumentException>("comparisonType", () => "foo".EndsWith("", StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => "foo".EndsWith("", StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type
        }
        ...
        public static void TestEquals(string s1, object obj, StringComparison comparisonType, bool expected)
        {
            ...
            if (s1 != null && comparisonType == StringComparison.Ordinal)
            {
        ...
        {
            Assert.Throws<ArgumentException>("comparisonType", () => string.Equals("a", "b", StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => string.Equals("a", "b", StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type

            Assert.Throws<ArgumentException>("comparisonType", () => "a".Equals("a", StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => "b".Equals("a", StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type
        }
        ...
        public static void TestFormat_Invalid()
        {
            ...
            Assert.Throws<ArgumentNullException>("format", () => string.Format(null, obj1)); // Format is null
            Assert.Throws<ArgumentNullException>("format", () => string.Format(null, obj1, obj2)); // Format is null            
            Assert.Throws<ArgumentNullException>("format", () => string.Format(null, obj1, obj2, obj3)); // Format is null
            Assert.Throws<ArgumentNullException>("format", () => string.Format(null, obj1, obj2, obj3, obj4)); // Format is null
            Assert.Throws<ArgumentNullException>("format", () => string.Format(null, (object[])null)); // Args and format are null
            Assert.Throws<ArgumentNullException>("args", () => string.Format("", null)); // Args is null

            Assert.Throws<ArgumentNullException>("format", () => string.Format(formatter, null, obj1)); // Format is null
            Assert.Throws<ArgumentNullException>("format", () => string.Format(formatter, null, obj1, obj2)); // Format is null
            Assert.Throws<ArgumentNullException>("format", () => string.Format(formatter, null, obj1, obj2, obj3)); // Format is null
            Assert.Throws<ArgumentNullException>("args", () => string.Format(formatter, "", null)); // Args is null
            Assert.Throws<ArgumentNullException>("format", () => string.Format(formatter, null, null)); // Args and format are null
        ...
        [Theory, MemberData("AllSubstringsAndComparisons", new object[] { "abcde" })]
        ...
        {
            Assert.Throws<ArgumentNullException>("value", () => "foo".IndexOf(null)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => "foo".IndexOf(null, 0)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => "foo".IndexOf(null, 0, 0)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => "foo".IndexOf(null, 0, StringComparison.CurrentCulture)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => "foo".IndexOf(null, 0, 0, StringComparison.CurrentCulture)); // Value is null

            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf("o", -1)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf('o', -1)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf("o", -1, 0)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf('o', -1, 0)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf("o", -1, StringComparison.CurrentCulture)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf("o", -1, 0, StringComparison.CurrentCulture)); // Start index < 0

            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf("o", 4)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf('o', 4)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf("o", 4, 0)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf('o', 4, 0)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf("o", 4, 0, StringComparison.CurrentCulture)); // Start index > string.length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".IndexOf("o", 4, 0, StringComparison.CurrentCulture)); // Start index > string.Length

            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".IndexOf("o", 0, -1)); // Count < 0
            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".IndexOf('o', 0, -1)); // Count < 0
            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".IndexOf("o", 0, -1, StringComparison.CurrentCulture)); // Count < 0

            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".IndexOf("o", 0, 4)); // Count > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".IndexOf('o', 0, 4)); // Count > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".IndexOf("o", 0, 4, StringComparison.CurrentCulture)); // Count > string.Length

            Assert.Throws<ArgumentException>("comparisonType", () => "foo".IndexOf("o", StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => "foo".IndexOf("o", StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type

            Assert.Throws<ArgumentException>("comparisonType", () => "foo".IndexOf("o", 0, StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => "foo".IndexOf("o", 0, StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type

            Assert.Throws<ArgumentException>("comparisonType", () => "foo".IndexOf("o", 0, 0, StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => "foo".IndexOf("o", 0, 0, StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => "foo".IndexOfAny(null));
            Assert.Throws<ArgumentNullException>(() => "foo".IndexOfAny(null, 0));
            Assert.Throws<ArgumentNullException>(() => "foo".IndexOfAny(null, 0, 0));

            Assert.Throws<ArgumentOutOfRangeException>(() => "foo".IndexOfAny(new char[] { 'o' }, -1)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => "foo".IndexOfAny(new char[] { 'o' }, -1, 0)); // Start index < 0

            Assert.Throws<ArgumentOutOfRangeException>(() => "foo".IndexOfAny(new char[] { 'o' }, 4)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>(() => "foo".IndexOfAny(new char[] { 'o' }, 4, 0)); // Start index > string.Length
            
            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".IndexOfAny(new char[] { 'o' }, 0, -1)); // Count < 0
            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".IndexOfAny(new char[] { 'o' }, 0, 4)); // Count > string.Length

            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".IndexOfAny(new char[] { 'o' }, 3, 1)); // Start index + count > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".IndexOfAny(new char[] { 'o' }, 2, 2)); // Start index + count > string.Length
        }
        ...
        public static IEnumerable<object[]> IsNullOrWhitespaceTestData()
        {
        ...
        [Theory, MemberData("IsNullOrWhitespaceTestData")]
        ...
        {
            Assert.Throws<ArgumentNullException>("value", () => string.Join("$$", null)); // Values is null
            Assert.Throws<ArgumentNullException>("value", () => string.Join("$$", null, 0, 0)); // Values is null

            Assert.Throws<ArgumentNullException>("values", () => string.Join("|", (IEnumerable<string>)null)); // Values is null
        ...
        [Theory, MemberData("Join_ObjectArray_TestData")]
        ...
        [Fact]
        public static void TestJoin_ObjectArray_Invalid()
        {
            Assert.Throws<ArgumentNullException>("values", () => string.Join("$$", (object[])null));
            Assert.Throws<ArgumentNullException>("values", () => string.Join("--", (IEnumerable<object>)null)); // Values is null
        }
        ...
        public static void TestLastIndexOf_Invalid()
        {
            ...
            Assert.Throws<ArgumentNullException>("value", () => s.LastIndexOf(null)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => s.LastIndexOf(null, StringComparison.CurrentCulture)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => s.LastIndexOf(null, 0)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => s.LastIndexOf(null, 0, 0)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => s.LastIndexOf(null, 0, 0, StringComparison.CurrentCulture)); // Value is null

            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf('a', -1)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf('a', -1, 0)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf("a", -1)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf("a", -1, StringComparison.CurrentCulture)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf("a", -1, 0)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf("a", -1, 0, StringComparison.CurrentCulture)); // Start index < 0

            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf('a', s.Length + 1)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf('a', s.Length + 1, 0)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf("a", s.Length + 1)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf("a", s.Length + 1, StringComparison.CurrentCulture)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf("a", s.Length + 1, 0)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.LastIndexOf("a", s.Length + 1, 0, StringComparison.CurrentCulture)); // Start index > string.Length

            Assert.Throws<ArgumentOutOfRangeException>("count", () => s.LastIndexOf('a', 0, -1)); // Count < 0
            Assert.Throws<ArgumentOutOfRangeException>("count", () => s.LastIndexOf("a", 0, -1)); // Count < 0
            Assert.Throws<ArgumentOutOfRangeException>("count", () => s.LastIndexOf("a", 0, -1, StringComparison.CurrentCulture)); // Count < 0

            Assert.Throws<ArgumentOutOfRangeException>("count", () => s.LastIndexOf('a', 0, s.Length + 2)); // Start index - count + 1 < 0
            Assert.Throws<ArgumentOutOfRangeException>("count", () => s.LastIndexOf("a", 0, s.Length + 2)); // Start index - count + 1 < 0
            Assert.Throws<ArgumentOutOfRangeException>("count", () => s.LastIndexOf("a", 0, s.Length + 2, StringComparison.CurrentCulture)); // Start index - count + 1 < 0
            
            Assert.Throws<ArgumentException>("comparisonType", () => s.LastIndexOf("a", StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => s.LastIndexOf("a", StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => s.LastIndexOf("a", 0, StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => s.LastIndexOf("a", 0, StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => s.LastIndexOf("a", 0, 0, StringComparison.CurrentCulture - 1)); // Invalid comparison type
            Assert.Throws<ArgumentException>("comparisonType", () => s.LastIndexOf("a", 0, 0, StringComparison.OrdinalIgnoreCase + 1)); // Invalid comparison type
        }
        ...
        public static void TestLastIndexOfAny_Invalid()
        {
            ...
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".LastIndexOfAny(new char[] { 'o' }, -1)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".LastIndexOfAny(new char[] { 'o' }, -1, 0)); // Start index < 0

            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".LastIndexOfAny(new char[] { 'o' }, 4)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".LastIndexOfAny(new char[] { 'o' }, 4, 0)); // Start index > string.Length

            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".LastIndexOfAny(new char[] { 'o' }, 0, -1)); // Count < 0
            Assert.Throws<ArgumentOutOfRangeException>("count", () => "foo".LastIndexOfAny(new char[] { 'o' }, 0, 4)); // Count > string.Length

            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".LastIndexOfAny(new char[] { 'o' }, 3, 1)); // Start index + count > string.Length
        }
        ...
        [Fact]
        public static void TestPadLeft_Invalid()
        {
        ...
        [Fact]
        public static void TestPadRight_Invalid()
        {
        ...
        public static void TestRemove_Invalid()
        {
            ...
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.Remove(-1)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.Remove(-1, 0)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => s.Remove(s.Length)); // Start index >= string.Length


            Assert.Throws<ArgumentOutOfRangeException>("count", () => s.Remove(0, -1)); // Count < 0

            Assert.Throws<ArgumentOutOfRangeException>("count", () => s.Remove(0, s.Length + 1)); // Start index + count > string.Length
        ...
        public static void TestStartsWith_Invalid()
        {
            ...
            Assert.Throws<ArgumentNullException>("value", () => s.StartsWith(null)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => s.StartsWith(null, StringComparison.CurrentCultureIgnoreCase)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => s.StartsWith(null, StringComparison.Ordinal)); // Value is null
            Assert.Throws<ArgumentNullException>("value", () => s.StartsWith(null, StringComparison.OrdinalIgnoreCase)); // Value is null
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".Substring(-1)); // Start index < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".Substring(4)); // Start index > string.Length

            Assert.Throws<ArgumentOutOfRangeException>("length", () => "foo".Substring(0, -1)); // Length < 0
            Assert.Throws<ArgumentOutOfRangeException>("length", () => "foo".Substring(0, 4)); // Length > string.Length

            Assert.Throws<ArgumentOutOfRangeException>("length", () => "foo".Substring(3, 2)); // Start index + length > string.Length
        }
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".ToCharArray(-1, 0)); // Start index < 0

            Assert.Throws<ArgumentOutOfRangeException>("length", () => "foo".ToCharArray(0, -1)); // Length < 0
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".ToCharArray(0, 4)); // Length > string.Length

            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".ToCharArray(4, 0)); // Start index > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".ToCharArray(3, 1)); // Start index + length > string.Length
            Assert.Throws<ArgumentOutOfRangeException>("startIndex", () => "foo".ToCharArray(2, 2)); // Start index + length > string.Length
        }
