...
namespace System.Collections.Concurrent
{
    ...
    public class ConcurrentQueue<T> : IProducerConsumerCollection<T>, IReadOnlyCollection<T>
    {
        ...
        private void InitializeFromCollection(IEnumerable<T> collection)
        {
            ...
            {
                Debug.Assert(index >= 0 && index < SegmentSize);
...
#pragma warning disable 0420 // No warning for Interlocked.xxx if compiled with new managed compiler (Roslyn)
        ...
        private List<T> ToList()
        {
            ...
            try
            {
                ...
                {
                    head.AddToList(list, headLow, SegmentSize - 1);
                    Segment curr = head.Next;
                    while (curr != tail)
                    {
                        curr.AddToList(list, 0, SegmentSize - 1);
                        curr = curr.Next;
                    }
                    // Add tail segment
        ...
        private IEnumerator<T> GetEnumerator(Segment head, Segment tail, int headLow, int tailHigh)
        {
            try
            {
                ...
                else
                {
                    ...
                    for (int i = headLow; i < SegmentSize; i++)
                    {
                        // If the position is reserved by an Enqueue operation, but the value is not written into,
                        // spin until the value is available.
                        spin.Reset();
        ...
        private class Segment
        {
            ...
            internal Segment(long index, ConcurrentQueue<T> source)
            {
                ...
                _state = new VolatileBool[SegmentSize]; // All initialized to false
                _high = -1;
            ...
            {
                Debug.Assert(_high < SegmentSize - 1);
                _high++;
                _array[_high] = value;
            ...
            internal bool TryAppend(T value)
            {
                ...
                int newhigh = SegmentSize; // Initial value set to be over the boundary
...
#pragma warning restore 0420
            ...
            internal int Low
            {
                get
                {
                    return Math.Min(_low, SegmentSize);
                }
            }
            ...
            internal int High
            {
                get
                {
                    ...
                    // SegmentSize-1 as the logical position
                    return Math.Min(_high, SegmentSize - 1);
                }
            }
        }
