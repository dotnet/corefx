...
namespace System.Composition.TypedParts.Discovery
{
    ...
    {
        private static readonly MethodInfo _ActivatorInvoke = typeof(CompositeActivator).GetRuntimeMethod("Invoke", new[] { typeof(LifetimeContext), typeof(CompositionOperation) });
        ...
        protected override ExportDescriptor GetExportDescriptor(CompositeActivator partActivator)
        {
            ...
            var activator = Expression.Lambda<CompositeActivator>(
                Expression.Property(
                    Expression.Convert(Expression.Call(Expression.Constant(partActivator), _ActivatorInvoke, args), _property.DeclaringType),
                    _property),
                args);
