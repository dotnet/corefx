...
namespace System.Xml
{
    internal partial class DtdParser : IDtdParser
    {
        ...
        private Token ScanDoctype2()
        {
            switch (_chars[_curPos])
            {
                ...
                case '>':
                    ...
                    return Token.GreaterThan;
                default:
                    Throw(_curPos, Res.Xml_ExpectSubOrClose);
                    return Token.None;
            }
        }
        ...
        private Token ScanElement4()
        {
            ...
            if (_whitespaceSeen)
            {
                Throw(_curPos, Res.Xml_ExpectNoWhitespace);
            }
            _curPos++;
            return t;
        }
        ...
        private Token ScanElement5()
        {
            switch (_chars[_curPos])
            {
                ...
                case '>':
                    ...
                    return Token.GreaterThan;
                default:
                    Throw(_curPos, Res.Xml_ExpectOp);
                    return Token.None;
            }
        }
        ...
        private XmlQualifiedName GetNameQualified(bool canHavePrefix)
        {
            ...
            else
            {
                ...
                {
                    Throw(_tokenStartPos, Res.Xml_ColonInLocalName, GetNameString());
                    return null;
                }
            }
        }
        ...
        private SchemaEntity VerifyEntityReference(XmlQualifiedName entityName, bool paramEntity, bool mustBeDeclared, bool inAttribute)
        {
            ...
            if (entity == null)
            {
                if (paramEntity)
                {
                    ...
                    if (_validate)
                    {
                        SendValidationEvent(_curPos - entityName.Name.Length - 1, XmlSeverityType.Error, Res.Xml_UndeclaredParEntity, entityName.Name);
                    }
#endif
                }
                else if (mustBeDeclared)
                {
                    if (!ParsingInternalSubset)
                    {
                        ...
                        if (_validate)
                        {
                            SendValidationEvent(_curPos - entityName.Name.Length - 1, XmlSeverityType.Error, Res.Xml_UndeclaredEntity, entityName.Name);
                        }
#endif
                    }
                    else
                    {
                        Throw(_curPos - entityName.Name.Length - 1, Res.Xml_UndeclaredEntity, entityName.Name);
                    }
                }
                return null;
            }
            ...
            {
                Throw(_curPos - entityName.Name.Length - 1, Res.Xml_UnparsedEntityRef, entityName.Name);
            }

            if (inAttribute && entity.IsExternal)
            {
                Throw(_curPos - entityName.Name.Length - 1, Res.Xml_ExternalEntityInAttValue, entityName.Name);
            }
        ...
        private void OnUnexpectedError()
        {
            ...
            Throw(_curPos, Res.Xml_InternalError);
        }
        ...
        private void ThrowUnexpectedToken(int pos, string expectedToken1, string expectedToken2)
        {
            ...
            {
                Throw(_curPos, Res.Xml_UnexpectedTokens2, new string[3] { unexpectedToken, expectedToken1, expectedToken2 });
            }
            else
            {
                Throw(_curPos, Res.Xml_UnexpectedTokenEx, new string[2] { unexpectedToken, expectedToken1 });
            }
        }
