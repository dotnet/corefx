...
namespace System.Composition.Convention
{
    ...
    public class PartConventionBuilder
    {
        ...
        private readonly static List<Attribute> _ImportingConstructorList = new List<Attribute>() { new ImportingConstructorAttribute() };
        private readonly static Type _ExportAttributeType = typeof(ExportAttribute);
        ...
        static bool MemberHasExportMetadata(MemberInfo member)
        {
            foreach (var attr in member.GetAttributes<Attribute>())
            {
                ...
                else
                {
                    ...
                    if (attrType != PartConventionBuilder._ExportAttributeType && attrType.GetTypeInfo().IsAttributeDefined<MetadataAttributeAttribute>(true))
                    {
                        return true;
                    }
                }
