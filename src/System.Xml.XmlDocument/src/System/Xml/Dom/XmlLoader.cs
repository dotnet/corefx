...
namespace System.Xml
{
    internal class XmlLoader
    {
        ...
        internal void Load(XmlDocument doc, XmlReader reader, bool preserveWhitespace)
        {
            ...
            if (this._reader.ReadState != ReadState.Interactive)
            {
        ...
        {
            Debug.Assert(this._reader != null);
        ...
        private XmlEntityReference LoadEntityReferenceNode(bool direct)
        {
            ...
            XmlEntityReference eref = direct ? new XmlEntityReference(_reader.Name, this._doc) : _doc.CreateEntityReference(_reader.Name);
            ...
            do
            {
                ...
                switch (r.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        XmlElement element = new XmlElement(_reader.Prefix, _reader.LocalName, _reader.NamespaceURI, this._doc);
                        element.IsEmpty = fEmptyElement;
                    ...
                    case XmlNodeType.SignificantWhitespace:
                        node = new XmlSignificantWhitespace(_reader.Value, this._doc);
                        break;

                    case XmlNodeType.Whitespace:
                        if (_preserveWhitespace)
                        {
                            node = new XmlWhitespace(_reader.Value, this._doc);
                        }
                        else
                        {
                            continue;
                    ...
                    case XmlNodeType.Text:
                        node = new XmlText(_reader.Value, this._doc);
                        break;

                    case XmlNodeType.CDATA:
                        node = new XmlCDataSection(_reader.Value, this._doc);
                        break;

                    case XmlNodeType.ProcessingInstruction:
                        node = new XmlProcessingInstruction(_reader.Name, _reader.Value, this._doc);
                        break;

                    case XmlNodeType.Comment:
                        node = new XmlComment(_reader.Value, this._doc);
                        break;
        ...
        private XmlAttribute LoadAttributeNodeDirect()
        {
            ...
            if (r.IsDefault)
            {
                XmlUnspecifiedAttribute defattr = new XmlUnspecifiedAttribute(r.Prefix, r.LocalName, r.NamespaceURI, this._doc);
            ...
            {
                attr = new XmlAttribute(r.Prefix, r.LocalName, r.NamespaceURI, this._doc);
...
#pragma warning restore 618
        ...
        private XmlParserContext GetContext(XmlNode node)
        {
            ...
            XmlDocumentType docType = this._doc.DocumentType;
            String baseURI = this._doc.BaseURI;
            //constructing xmlnamespace
            HashSet<string> prefixes = new HashSet<string>();
            XmlNameTable nt = this._doc.NameTable;
        ...
        internal XmlNamespaceManager ParsePartialContent(XmlNode parentNode, string innerxmltext, XmlNodeType nt)
        {
            ...
            this._doc = parentNode.OwnerDocument;
            Debug.Assert(this._doc != null);
            XmlParserContext pc = GetContext(parentNode);
            this._reader = CreateInnerXmlReader(innerxmltext, nt, pc, this._doc);
            try
            {
                this._preserveWhitespace = true;
