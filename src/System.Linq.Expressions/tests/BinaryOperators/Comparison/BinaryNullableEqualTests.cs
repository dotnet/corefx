...
namespace System.Linq.Expressions.Tests
{
    public static class BinaryNullableEqualTests
    {
        ...
        [Theory, ClassData(typeof(CompilationTypes))]
        public static void CheckNullableEqualityWithConstantCheck(bool useInterpreter)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                for (int j = 0; j < array.Length; j++)
                {
                    VerifyNullableEqualityWithConstantCheck(array[i], array[j], useInterpreter, isNull[i] == isNull[j]);
                }
            }
        }
        ...
        private static void VerifyNullableEqualityWithConstantCheck(Expression a, Expression b, bool useInterpreter, bool expected)
        {
            Expression<Func<bool>> e =
                Expression.Lambda<Func<bool>>(
