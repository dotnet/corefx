...
namespace Tests.Integration
{
    public class RejectionTests
    {
        ...
        public void Rejection_ExtensionLightUp_AddedViaBatch()
        {
            ...
            Assert.Equal(0, importer.Extensions.Length); // "Should have 0 extensions");

            container.ComposeExportedValue<int>("IExtension.IdValue", 10);

            Assert.Equal(1, importer.Extensions.Length); // "Should have 1 extension");
            Assert.Equal(10, importer.Extensions[0].Id);

            container.ComposeExportedValue<int>("IExtension.IdValue2", 20);

            Assert.Equal(2, importer.Extensions.Length); // "Should have 2 extension");
            Assert.Equal(10, importer.Extensions[0].Id);
        ...
        public void Rejection_ExtensionLightUp_AddedViaCatalog()
        {
            ...
            Assert.Equal(0, importer.Extensions.Length); // "Should have 0 extensions after ext1 added without dependency");

            catalog.Catalogs.Add(ext2Cat);

            Assert.Equal(0, importer.Extensions.Length); // "Should have 0 extensions after ext2 added without dependency");
        ...
        public void Rejection_Resurrection()
        {
            ...
            Assert.Equal(0, exports1.Count()); // "Catalog entry should be rejected");

            container.ComposeParts(new NoImportPart());

            var exports2 = container.GetExportedValues<Needy>();
            Assert.Equal(1, exports2.Count()); // "Catalog entry should be ressurrected");
        }
        ...
        public void Rejection_BatchSatisfiesBatch()
        {
            ...
            Assert.IsType<SingleImpl>(needy.SingleImport); // "Import not satisifed as expected");
        }
...
[Fact]
        ...
        public void Rejection_TheClemensLoop()
        {
            ...
            Assert.Equal(0, exportsA.Count()); // "Catalog ILoopA entries should be rejected");
            Assert.Equal(0, exportsB.Count()); // "Catalog ILoopB entries should be rejected");

            // These assertions would prove solution two
            //Assert.Equal(1, exportsA.Count, "Only noe ILoopA entry should not be rejected");
            //Assert.Equal(1, exportsB.Count, "Only noe ILoopB entry should not be rejected");
        }
