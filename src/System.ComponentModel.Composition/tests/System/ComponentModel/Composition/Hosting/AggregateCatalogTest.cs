...
[Fact]
        ...
        public void EnumeratePartsProperty_ShouldSucceed()
        {
            using (var catalog = new AggregateCatalog(
                ...
                new TypeCatalog(typeof(SharedPartStuff))))
            {
                Assert.True(catalog.Catalogs.Count() == 6, "Initialise AggregateCatalog gets right number of catalogs");
                Assert.True(catalog.Parts.Count() == 6, "Initialise AggregateCatalog gets right number of catalogs");
            }
       }
        ...
        public void MutableCatalogNotifications()
        {
            ...
            Assert.True(catalog.Catalogs.Count == 0, "Add/Remove/Clear -- PartsAsCollection.Count is now 0");
            ...
            catalog.Catalogs.Add(typePartCatalog);
            Assert.True(catalog.Catalogs.Count == 1, "Add -- Catalogs.Count is now 1");
            Assert.True(changedStep == 2, "Add -- Changed must be fired after");
            ...
            catalog.Catalogs.Remove(typePartCatalog);
            Assert.True(catalog.Catalogs.Count == 0, "Add -- Catalogs.Count is now 0");
            Assert.True(changedStep == 2, "Remove -- Changed must be fired after");
...
//Now Add it back
            ...
            Assert.True(removed == false, "Remove -- correctly returned false");
            Assert.True(changedStep == 0, "Remove -- Changed should not fire if nothing changed");
            ...
            Assert.True(catalog.Catalogs.Count == 2, "Add -- Catalogs.Count is now 1");
            Assert.True(changedStep == 4, "Add -- Changing must be fired after");
            ...
            Assert.True(catalog.Catalogs.Count == 6, "Add -- Catalogs.Count is now 1");
            Assert.True(changedStep == 12, "Add -- Changing must be fired after");

            removed = catalog.Catalogs.Remove(typePartCatalog3);
            Assert.True(catalog.Catalogs.Count == 5, "Add -- Catalogs.Count is now 5");
            Assert.True(removed == true, "Remove should have succeeded");
            Assert.True(changedStep == 14, "Remove -- Changed must be fired after");
            ...
            Assert.True(catalog.Catalogs.Count == 0, "Add -- Catalogs.Count is now 0");
            Assert.True(removed == true, "Remove should have succeeded");
            Assert.True(changedStep == 24, "Remove -- Changing must be fired after");
            ...
            catalog.Catalogs.Add(typePartCatalog4);
            catalog.Catalogs.Add(typePartCatalog5);
            Assert.True(catalog.Catalogs.Count == 6, "Add -- Catalogs.Count should be 6");
            Assert.True(changedStep == 12, "Add -- Changing must be fired after");

            catalog.Catalogs.Clear();
            Assert.True(catalog.Catalogs.Count == 0, "Add -- Catalogs.Count should be 0");
            ...
            catalog.Catalogs.Add(typePartCatalog);
            Assert.True(catalog.Catalogs.Count == 1, "Add -- Catalogs.Count is now 1");
            Assert.True(changedStep == 2, "Add handler 1 -- Changed must be fired after");
            Assert.True(changedStep2 == 98, "Add handler 2 -- Changed must be fired after");

            catalog.Catalogs.Add(typePartCatalog1);
            Assert.True(catalog.Catalogs.Count == 2, "Add -- Catalogs.Count is now 1");
            Assert.True(changedStep == 4, "Add handler 1 -- Changed must be fired after");
            Assert.True(changedStep2 == 96, "Add handler 2 -- Changed must be firedafter");

            catalog.Catalogs.Remove(typePartCatalog);
            Assert.True(catalog.Catalogs.Count == 1, "Add -- PartsAsCollection.Count is now 1");
            Assert.True(changedStep == 6, "Add handler 1 -- Changed must be fired and fired after");
            Assert.True(changedStep2 == 94, "Add handler 2 -- Changed must be fired and fired after");

            catalog.Catalogs.Clear();
            Assert.True(catalog.Catalogs.Count == 0, "Add -- PartsAsCollection.Count is now 0");
            Assert.True(changedStep == 8, "Add handler 1 -- Changed must be fired after");
            Assert.True(changedStep2 == 92, "Add handler 2 -- Changed must be fired after");
        ...
        public void DisposeAggregatingCatalog()
        {
            ...
            Assert.True(changedNotification == 0, "No changed notifications");
        ...
        public void MutableMultithreadedEnumerations()
        {
            ...
            ThreadStart func = delegate()
            {
                ...
                for (int i = 0; i < 100; i++)
                {
                    ...
                    Assert.True(catalog.Catalogs.Count >= 6, "Catalogs Collection must be at least 6 big");
                    ...
                    for (int k = 0; k < 5; k++)
                    {
                        ...
                        Assert.True(j >= 6, "Catalogs Collection must be at least 6 big");
...
catalog.Catalogs.Remove(typePart);
            ...
            Assert.True(catalog.Catalogs.Count == 0, "Collection must be empty");
        }
...
public interface IFoo { }
        ...
        public class CatalogListener
        {
            ...
            public CatalogListener(AggregateCatalog watchCatalog, AggregateCatalog modifiedCatalog)
            {
                ...
                _watchedCatalog = watchCatalog;
                _modifiedCatalog = modifiedCatalog;
            }

            public void VerifyAdd(ComposablePartCatalog catalogToAdd, params Type[] expectedTypesAdded)
            {
                _expectedAdds = GetDisplayNames(expectedTypesAdded);

                _modifiedCatalog.Catalogs.Add(catalogToAdd);

                Assert.True(_changingEventCount == 1, "Changing event should have been called");
                Assert.True(_changedEventCount == 1, "Changed event should have been called");
            ...
            {
                _expectedAdds = null;
                _expectedRemoves = GetDisplayNames(expectedTypesRemoved);

                _modifiedCatalog.Catalogs.Remove(catalogToRemove);
            ...
            public void VerifyClear(params Type[] expectedTypesRemoved)
            {
                ...
                _modifiedCatalog.Catalogs.Clear();
            ...
            {
                Assert.True(_expectedAdds != null || _expectedRemoves != null);

                if (_expectedAdds == null)
                {
                    EnumerableAssert.IsEmpty(args.AddedDefinitions);
                }
                else
                {
                    EnumerableAssert.AreSequenceEqual(_expectedAdds, GetDisplayNames(args.AddedDefinitions));
                }

                if (_expectedRemoves == null)
                {
                    EnumerableAssert.IsEmpty(args.RemovedDefinitions);
                }
                else
                {
                    EnumerableAssert.AreSequenceEqual(_expectedRemoves, GetDisplayNames(args.RemovedDefinitions));
                }
            ...
            public void OnChanged(object sender, ComposablePartCatalogChangeEventArgs args)
            {
                ...
                Assert.True(ContainsChanges(), "The catalog should contain the changes");
            ...
            {
                var allParts = GetDisplayNames(_watchedCatalog.Parts);

                if (_expectedAdds != null)
                {
                    foreach (var add in _expectedAdds)
                    {
                ...
                if (_expectedRemoves != null)
                {
                    foreach (var remove in _expectedRemoves)
                    {
            ...
            {
                _expectedAdds = null;
                _expectedRemoves = null;
                _changedEventCount = 0;
                _changingEventCount = 0;
            }
