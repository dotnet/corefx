...
namespace System.ComponentModel.Composition
{
    public class CompositionContainerTests
    {
        ...
        [Fact(Skip = "WorkItem(579990)")] // NullReferenceException
        ...
        public void AddPart1_ImportOnlyPart_ShouldNotGetGarbageCollected()
        {
            ...
            Assert.NotNull(weakRef.Target); // "Import only part should not have been collected!");
...
Assert.Equal(1, importer.Value);
        ...
        public void TryComposeSimple()
        {
            ...
            Assert.Equal(42, importer.Value); // "Expected value imported from export");
        }
        ...
        public void TryComposeSimpleFail()
        {
            ...
            Assert.Equal(0, importer.Value); // "Expected default value to remain");
        }
        ...
        public void ComposeDisposableChildContainer()
        {
            ...
            Assert.Equal(42, outerImporter.Value); // "Expected value imported from export");
            ...
            Assert.Equal(42, innerImporter.Value); // "Expected value imported from export");
            Assert.Equal(42, outerImporter.Value); // "Expected value imported from export");
            ...
            outerContainer.Compose(outerBatch);
            Assert.Equal(-5, innerImporter.Value); // "Expected update value imported from export");
            Assert.Equal(-5, outerImporter.Value); // "Expected updated value imported from export");
            ...
            outerContainer.Compose(outerBatch);
            Assert.Equal(500, outerImporter.Value); // "Expected updated value imported from export");
            Assert.Equal(-5, innerImporter.Value); // "Expected value not updated");
        }
        ...
        public void RemoveValueTest()
        {
            ...
            Assert.Equal("hello", result); // "Should get the correct value");
        ...
        public void ExportsChanged_ExportNothing_ShouldNotFireExportsChanged()
        {
            ...
            {
                Assert.False(true); // "Event should not be fired!");
            };
        ...
        public void ExportsChanged_ExportAdded_ShouldFireExportsChanged()
        {
            ...
            container.ExportsChanged += (sender, args) =>
            {
                ...
                Assert.Null(changedNames); // "Ensure this event only fires once!");
...
[Fact]
        public void ExportsChanged_AddExportOnChild_ShouldFireExportsChangedOnChildOnly()
        {
            ...
            parent.ExportsChanged += (sender, args) =>
            {
                Assert.False(true); // "Should not fire on parent container!!");
            };
...
var typeCatalog = new TypeCatalog(typeof(SimpleExporter));
        ...
        public void RawExportTests()
        {
            ...
            Assert.Equal(1, export.Value); // "Should be the value I put in...");
        }
        ...
        {
            Assert.False(true); // "This scenario (required metadata warnings) no longer works, see 468388");
        ...
        public void ConverterExportTests()
        {
            ...
            Assert.Equal(1, export.Value); // "Should be the value I put in...");
            Assert.NotNull(export.Metadata); // "Should have metadata (as an object)");
        }
