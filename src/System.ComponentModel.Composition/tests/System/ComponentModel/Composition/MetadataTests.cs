...
namespace System.ComponentModel.Composition
{
    public class MetadataTests
    {
        ...
        public void ValidMetadataTest()
        {
            ...
            Assert.Equal(2, metadataFoo.Count()); // "There are should be two items in the metadata foo's collection");
            ...
            Assert.Equal("world", typeVi.Metadata["hello"]); // "The single item metadata should be present");
            Assert.Equal("GoodOneValue2", typeVi.Metadata["GoodOne2"]); // "The metadata supplied by strong attribute should also be present");

            var metadataAcme = typeVi.Metadata["acme"] as IList<object>;
            Assert.Equal(2, metadataAcme.Count()); // "There are should be two items in the metadata acme's collection");
            ...
            Assert.Equal(2, metadataBar.Count()); // "There are should be two items in the metadata bar's collection");
            ...
            Assert.Equal("hello", memberVi.Metadata["world"]); // "The single item metadata should be present");
            Assert.Equal("GoodOneValue2", memberVi.Metadata["GoodOne2"]); // "The metadata supplied by strong attribute should also be present");
        ...
        private void ValidMetadataDiscoveredByCatalog(CompositionContainer container)
        {
            ...
            Assert.True(metadataFoo.Contains("bar1"), "The metadata collection should include value 'bar1'");
            Assert.True(metadataFoo.Contains("bar2"), "The metadata collection should include value 'bar2'");
            Assert.Equal("world", export1.Metadata["hello"]); // "The single item metadata should also be present");
            Assert.Equal("GoodOneValue2", export1.Metadata["GoodOne2"]); // "The metadata supplied by strong attribute should also be present");
            ...
            Assert.Equal(2, metadataBar.Count()); // "There are should be two items in the metadata foo's collection");
            Assert.True(metadataBar.Contains("foo1"), "The metadata collection should include value 'foo1'");
            Assert.True(metadataBar.Contains("foo2"), "The metadata collection should include value 'foo2'");
            Assert.Equal("hello", export2.Metadata["world"]); // "The single item metadata should also be present");
            Assert.Equal("GoodOneValue2", export2.Metadata["GoodOne2"]); // "The metadata supplied by strong attribute should also be present");
...
[AttributeUsage(AttributeTargets.All, AllowMultiple = true)]
        ...
        public void FailureImportForNoRequiredMetadatForExportCollection()
        {
            ...
            //Assert.Equal(1, result.Issues.Count, "There should be one issue reported");
            //Assert.True(result.Issues[0].Description.Contains("Foo"), "The missing required metadata is 'Foo'");
        }

        [Fact(Skip="WorkItem(472538)")]
        ...
        {
            Assert.False(true); // "This needs to be fixed, see: 472538"
        ...
        public void SelectiveImportBasedOnMetadataForExport()
        {
            ...
            //Assert.Equal(1, result.Issues.Count, "There should be one issue reported about the export who has no required metadata");
            //Assert.True(result.Issues[0].Description.Contains("Foo"), "The missing required metadata is 'Foo'");
        ...
        public void SelectiveImportBasedOnMetadataForExportCollection()
        {
            ...
            //Assert.Equal(1, result.Issues.Count, "There should be one issue reported however, about the export who has no required metadata");
            //Assert.True(result.Issues[0].Description.Contains("Foo"), "The missing required metadata is 'Foo'");
            //Assert.Equal(1, importer.ValueInfoCol.Count, "The export with required metadata should get bound");
...
[Fact(Skip="WorkItem(472538)")]
        ...
        public void ChildParentContainerTest3()
        {
            ...
            Assert.Equal(2, exports.Count()); // "There should be two from child and parent container each");
        }
        ...
        public void SelectiveImportBySTM_Export()
        {
            ...
            //Assert.Equal(1, result.Issues.Count, "There should be an issue reported about the export who has no required metadata");
            //Assert.NotNull(importer.ValueInfo, "The valid export should really get bound");
            //Assert.Equal("Bar", importer.ValueInfo.Metadata.Foo, "The value of metadata 'Foo' should be 'Bar'");
        }
        ...
        public void SelectiveImportBySTM_ExportCollection()
        {
            ...
            //Assert.Equal(1, result.Issues.Count, "There should be an issue reported about the export with no required metadata");
            //Assert.Equal(1, importer.ValueInfoCol.Count, "There should be only one export got bound");
            //Assert.Equal("Bar", importer.ValueInfoCol.First().Metadata.Foo, "The value of metadata 'Foo' should be 'Bar'");
        }
        ...
        public void SelectiveImportBySTMThroughCatalog2(CompositionContainer container)
        {
            ...
            //Assert.Equal("Bar", result1.Value.ValueInfo.Metadata.Foo, "The value of metadata 'Foo' should be 'Bar'");
            ...
            //Assert.Equal(1, result2.Value.ValueInfoCol.Count, "There should be only one export got bound");
            //Assert.Equal("Bar", result2.Value.ValueInfoCol.First().Metadata.Foo, "The value of metadata 'Foo' should be 'Bar'");
        }
