...
[Fact]
        ...
        public void SetImport_SetPostValueAndSetAgainOnInstance_ShouldSetProperty()
        {
            ...
            Assert.NotEqual(import.Value, 21); // "Value should NOT be set on live object until OnComposed");
            part.Activate();

            Assert.Equal(import.Value, 21); // "Value should be set on live object now");

            part.SetImport(importDef, CreateSimpleExports(42));

            Assert.NotEqual(import.Value, 42); // "Value should NOT be rebound on live object");

            part.Activate();

            Assert.Equal(import.Value, 42); // "Value should be set on live object now");
        }
