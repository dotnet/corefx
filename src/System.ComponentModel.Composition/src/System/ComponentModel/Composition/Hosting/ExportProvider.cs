        ...
        public IEnumerable<Export> GetExports(ImportDefinition definition, AtomicComposition atomicComposition)
        {
            ...
            switch(result)
            {
                ...
                case ExportCardinalityCheckResult.NoExports:
                    throw new ImportCardinalityMismatchException(SR.Format(CultureInfo.CurrentCulture, SR.CardinalityMismatch_NoExports, definition.ToString()));
                default:
                    if (result != ExportCardinalityCheckResult.TooManyExports)
                    {
                        throw new Exception(SR.Diagnostic_InternalExceptionMessage);
                    }
                    throw new ImportCardinalityMismatchException(SR.Format(CultureInfo.CurrentCulture, SR.CardinalityMismatch_TooManyExports_Constraint, definition.ToString()));
            }
        }
