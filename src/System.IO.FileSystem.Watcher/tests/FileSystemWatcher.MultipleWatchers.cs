...
namespace System.IO.Tests
{
    public class FileSystemWatcher_Multiple_Test : FileSystemWatcherTest
    {
        ...
        [OuterLoop]
        [Fact]
        public void FileSystemWatcher_File_Create_MultipleWatchers_NotAffectEachOther()
        {
            using (var watcher1 = new FileSystemWatcher(TestDirectory))
            using (var watcher2 = new FileSystemWatcher(TestDirectory))
            using (var watcher3 = new FileSystemWatcher(TestDirectory))
            {
                ...
                File.Create(fileName).Dispose();
                Assert.False(autoResetEvent1.WaitOne(WaitForExpectedEventTimeout_NoRetry));
        ...
        [OuterLoop]
        [Fact]
        public void FileSystemWatcher_File_Create_MultipleWatchers_ForceLoopRestart()
        {
            string fileName = Path.Combine(TestDirectory, "file");
            ...
            File.Create(fileName).Dispose(); 
            Assert.False(WaitHandle.WaitAll(autoResetEvents, WaitForExpectedEventTimeout_NoRetry));
        ...
        [OuterLoop]
        [Fact]
        public void  FileSystemWatcher_File_Changed_MultipleWatchers_NotAffectEachOther()
        {
            ...
            using (var watcher3 = new FileSystemWatcher(testDirectory.Path, Path.GetFileName(otherFile.Path)))
            {
                ...
                Assert.False(autoResetEvent3.WaitOne(WaitForExpectedEventTimeout_NoRetry));

                Directory.SetLastWriteTime(otherFile.Path, DateTime.Now + TimeSpan.FromSeconds(10));
                Assert.False(WaitHandle.WaitAll(new[] { autoResetEvent1, autoResetEvent2 }, WaitForExpectedEventTimeout_NoRetry));
                Assert.True(autoResetEvent3.WaitOne(WaitForExpectedEventTimeout_NoRetry));

                watcher1.EnableRaisingEvents = false;

                Directory.SetLastWriteTime(file.Path, DateTime.Now + TimeSpan.FromSeconds(10));
                Assert.False(WaitHandle.WaitAll(new[] { autoResetEvent1, autoResetEvent3 }, WaitForExpectedEventTimeout_NoRetry));
        ...
        [OuterLoop]
        [Fact]
        public void FileSystemWatcher_File_Delete_MultipleWatchers_NotAffectEachOther()
        {
            using (var watcher1 = new FileSystemWatcher(TestDirectory))
            using (var watcher2 = new FileSystemWatcher(TestDirectory))
            using (var watcher3 = new FileSystemWatcher(TestDirectory))
        ...
        public void FileSystemWatcher_File_Rename_MultipleWatchers_NotAffectEachOther()
        {
            using (var testDirectory = new TempDirectory(GetTestFilePath()))
            using (var file = new TempFile(Path.Combine(testDirectory.Path, "file")))
            using (var watcher1 = new FileSystemWatcher(testDirectory.Path, Path.GetFileName(file.Path)))
