...
#define XMLCHARTYPE_TEST(ch, mh)              xmlCharType.mh(ch)
#define XMLCHARTYPE_ISATTRIBUTEVALUECHAR(ch)  XMLCHARTYPE_TEST(ch, IsAttributeValueChar)
#define XMLCHARTYPE_ISTEXT(ch)                XMLCHARTYPE_TEST(ch, IsTextChar)
...
namespace System.Xml
{
    ...
    internal partial class _CLASS_NAME : XmlRawWriter
    {
        //
        // Fields
        //
        ...
        public _CLASS_NAME(TextWriter writer, XmlWriterSettings settings) : this(settings)
        {
            ...
            this.bufChars = new _BUFFER_TYPE[bufLen + OVERFLOW];
        ...
        public _CLASS_NAME(Stream stream, XmlWriterSettings settings) : this(settings)
        {
            ...
            _BUFFER = new _BUFFER_TYPE[bufLen + OVERFLOW];
        ...
        protected virtual void FlushBuffer()
        {
            try
            {
                ...
                if (!writeToNull)
                {
                    ...
                    stream.Write(_BUFFER, 1, bufPos - 1);
#else
                    ...
                    else
                    {
                        ...
                        writer.Write(_BUFFER, 1, bufPos - 1);
                    }
#endif
                }
            }
        ...
        private void EncodeChars(int startOffset, int endOffset, bool writeAllToStream)
        {
            ...
            while (startOffset < endOffset)
            {
                ...
                encoder.Convert(_BUFFER, startOffset, endOffset - startOffset, bufBytes, bufBytesUsed, bufBytes.Length - bufBytesUsed, false, out chEnc, out bEnc, out completed);
                startOffset += chEnc;
                bufBytesUsed += bEnc;
        ...
        private void FlushEncoder()
        {
            ...
            if (stream != null)
            {
                ...
                encoder.Convert(_BUFFER, 1, 0, bufBytes, 0, bufBytes.Length, true, out chEnc, out bEnc, out completed);
                if (bEnc != 0)
                {
...
#if _XML_UTF8_TEXT_WRITER
                    while (pDst < pDstEnd && (XMLCHARTYPE_ISATTRIBUTEVALUECHAR((char)(ch = *pSrc)) && ch <= 0x7F))
#else
                    while (pDst < pDstEnd && XMLCHARTYPE_ISATTRIBUTEVALUECHAR((char)(ch = *pSrc)))
#endif
                    {
...
#ifdef _XML_UTF8_TEXT_WRITER
                    while (pDst < pDstEnd && (XMLCHARTYPE_ISTEXT((char)(ch = *pSrc)) && ch <= 0x7F))
#else
                    while (pDst < pDstEnd && XMLCHARTYPE_ISTEXT((char)(ch = *pSrc)))
#endif
                    {
                        *pDst = (_BUFFER_TYPE)ch;
                        pDst++;
...
#ifdef _XML_UTF8_TEXT_WRITER
                    while (pDst < pDstEnd && (XMLCHARTYPE_ISTEXT((char)(ch = *pSrc)) && ch != stopChar && ch <= 0x7F))
#else
                    while (pDst < pDstEnd && (XMLCHARTYPE_ISTEXT((char)(ch = *pSrc)) && ch != stopChar))
#endif
                    {
                        *pDst = (_BUFFER_TYPE)ch;
                        pDst++;
        ...
        protected unsafe void WriteCDataSection(string text)
        {
            ...
            fixed (_BUFFER_TYPE* pDstBegin = _BUFFER)
            {
                ...
                for (;;)
                {
...
#ifdef _XML_UTF8_TEXT_WRITER
                    while (pDst < pDstEnd && (XMLCHARTYPE_ISATTRIBUTEVALUECHAR((char)(ch = *pSrc)) && ch != ']' && ch <= 0x7F))
#else
                    while (pDst < pDstEnd && (XMLCHARTYPE_ISATTRIBUTEVALUECHAR((char)(ch = *pSrc)) && ch != ']'))
#endif
                    {
                        *pDst = (_BUFFER_TYPE)ch;
                        pDst++;
                    ...
                    {
                        case '>':
                            if (hadDoubleBracket && pDst[-1] == (_BUFFER_TYPE)']')
            ...
            if (ch <= XmlCharType.SurHighEnd)
            {
                if (pSrc + 1 < pSrcEnd)
                {
                        ...
                        (LocalAppContextSwitches.DontThrowOnInvalidSurrogatePairs || lowChar <= XmlCharType.SurLowEnd))
                    {
                        ...
                        pDst += 4;
#else
                        pDst[0] = (_BUFFER_TYPE)ch;
                        pDst[1] = (_BUFFER_TYPE)lowChar;
                        pDst += 2;
#endif
    ...
    internal partial class _CLASS_NAME_INDENT : _CLASS_NAME
    {
        //
        // Fields
        //
