//------------------------------------------------------------------------------
// <copyright file="ActiveDirectorySchedule.cs" company="Microsoft">
 ...
 namespace System.DirectoryServices.ActiveDirectory {
    ...
    public class ActiveDirectorySchedule {
        ...
        public ActiveDirectorySchedule() 
        {
            ...
            utcOffSet = TimeZone.CurrentTimeZone.GetUtcOffset(DateTime.Now).Ticks/TimeSpan.TicksPerHour;
        }
        ...
        public ActiveDirectorySchedule(ActiveDirectorySchedule schedule) :this()
        {
            ...
            bool[] tmpSchedule = schedule.scheduleArray;
            for(int i = 0; i < 672; i++)
                scheduleArray[i] = tmpSchedule[i];
        }       
        ...
        public bool[, ,] RawSchedule {
            get {                
                ...
                for(int i = 0; i < 7; i++)
                    for(int j = 0; j < 24; j++)
                        for(int k = 0; k < 4; k++)
                            tmp[i,j,k] = scheduleArray[i*24*4 + j*4 + k];
                return tmp;
            }
            set {
                if(value == null)
        ...
        internal byte[] GetUnmanagedSchedule()
        {
            ...
            for(int i = 20; i < 188; i++)
            {
                ...
                if(scheduleArray[index])
                    val |= 1;
                if(scheduleArray[index + 1])
                    val |= 2;
                if(scheduleArray[index + 2])
                    val |= 4;
                if(scheduleArray[index + 3])
                    val |= 8;
                ...
                unmanagedScheduleIndex = i - (int)utcOffSet;
        ...
        internal void SetUnmanagedSchedule(byte[] unmanagedSchedule)
        {
            ...
            for(int i = 20; i < 188; i++)
            {
                ...
                if((val & 1) != 0)
                    scheduleArray[index] = true;

                if((val & 2) != 0)
                    scheduleArray[index + 1] = true;

                if((val & 4) != 0)
                    scheduleArray[index + 2] = true;

                if((val & 8) != 0)
                    scheduleArray[index + 3] = true;
            }
