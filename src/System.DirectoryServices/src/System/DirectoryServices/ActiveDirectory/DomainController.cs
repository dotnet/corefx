//------------------------------------------------------------------------------
// <copyright file="DomainController.cs" company="Microsoft">
...
namespace System.DirectoryServices.ActiveDirectory {
	...
	delegate bool SyncReplicaFromAllServersCallback(IntPtr data, IntPtr update);    
	...
	public class DomainController : DirectoryServer
	{
		...
		private string cachedComputerObjectName = null;
		private string cachedOSVersion = null;
		private double cachedNumericOSVersion = 0;
		private Forest currentForest = null;
		private Domain cachedDomain = null;
		private ActiveDirectoryRoleCollection cachedRoles = null;
		private bool dcInfoInitialized = false;
		...
		public override ReplicationCursorCollection GetReplicationCursors(string partition) {
			...
			// get the handle
			GetDSHandle();
			info = GetReplicationInfoHelper(dsHandle, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_CURSORS_3_FOR_NC, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_CURSORS_FOR_NC, partition, ref advanced, context, DirectoryContext.ADHandle);
			return ConstructReplicationCursors(dsHandle, advanced, info, partition, this, DirectoryContext.ADHandle);
		}
		...
		public override ReplicationOperationInformation GetReplicationOperationInformation() {
			...
			// get the handle
			GetDSHandle();
			info = GetReplicationInfoHelper(dsHandle, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_PENDING_OPS, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_PENDING_OPS, null, ref advanced, 0, DirectoryContext.ADHandle);
		...
		public override ReplicationNeighborCollection GetReplicationNeighbors(string partition) {
			...
			// get the handle
			GetDSHandle();
			info = GetReplicationInfoHelper(dsHandle, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_NEIGHBORS, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_NEIGHBORS, partition, ref advanced, 0, DirectoryContext.ADHandle);
		...
		public override ReplicationNeighborCollection GetAllReplicationNeighbors() {
			...
			// get the handle
			GetDSHandle();
			info = GetReplicationInfoHelper(dsHandle, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_NEIGHBORS, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_NEIGHBORS, null, ref advanced, 0, DirectoryContext.ADHandle);
			return ConstructNeighbors(info, this, DirectoryContext.ADHandle);
		}
		...
		public override ActiveDirectoryReplicationMetadata GetReplicationMetadata(string objectPath) {
			...
			// get the handle
			GetDSHandle();
			info = GetReplicationInfoHelper(dsHandle, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_METADATA_2_FOR_OBJ, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_METADATA_FOR_OBJ, objectPath, ref advanced, 0, DirectoryContext.ADHandle);
		...
		public override void SyncReplicaFromServer(string partition, string sourceServer) {
			...
			// get the dsHandle
			GetDSHandle();
			SyncReplicaHelper(dsHandle, false, partition, sourceServer, 0, DirectoryContext.ADHandle);
		}
		...
		public override void TriggerSyncReplicaFromNeighbors(string partition) {
			...
			// get the dsHandle
			GetDSHandle();
			SyncReplicaHelper(dsHandle, false, partition, null, DS_REPSYNC_ASYNCHRONOUS_OPERATION | DS_REPSYNC_ALL_SOURCES, DirectoryContext.ADHandle);
		}
		...
		public Forest Forest {
			get {
				...
				if (currentForest == null) {
					...
					currentForest = Forest.GetForest(forestContext);
				}
		...
		public string OSVersion {
			get {
				...
				if (cachedOSVersion == null) {
					...
					cachedOSVersion = (string) PropertyManager.GetPropertyValue(context, computerEntry, PropertyManager.OperatingSystem);
				}
		...
		internal double NumericOSVersion {
			get {
				...
				if (cachedNumericOSVersion == 0) {
					...
					cachedNumericOSVersion = (double)Double.Parse(osVersion, NumberFormatInfo.InvariantInfo);
				}
		...
		public Domain Domain { 
			get {
				...
				if (cachedDomain == null) {
					...
					cachedDomain = new Domain(domainContext, domainName);
				}
				return cachedDomain;
			}
		}
		...
		private void GetDomainControllerInfo() {
			...
			result = dsGetDomainControllerInfo(dsHandle, Domain.Name, dcInfoLevel, out dcCount, out dcInfoPtr);
			...
			if (result == 0) {
				try {
					...
					for (int i = 0; i < dcCount; i++) {
						if (dcInfoLevel == NativeMethods.DsDomainControllerInfoLevel3) {
							...
							if (domainControllerInfo3 != null)
							{
								if (Utils.Compare(domainControllerInfo3.dnsHostName, replicaName) == 0)
								{
									...
									cachedComputerObjectName = domainControllerInfo3.computerObjectName;
						...
						else { //NativeMethods.DsDomainControllerInfoLevel2
							...
							if (domainControllerInfo2 != null)
							{
								if (Utils.Compare(domainControllerInfo2.dnsHostName, replicaName) == 0)
								{
									...
									cachedComputerObjectName = domainControllerInfo2.computerObjectName;
		...
		internal ReplicationFailureCollection GetReplicationFailures(DS_REPL_INFO_TYPE type) {
			...
			// get the handle
			GetDSHandle();
			info = GetReplicationInfoHelper(dsHandle, (int)type, (int)type, null, ref advanced, 0, DirectoryContext.ADHandle);
		...
		private ArrayList GetRoles() {
			...
			result = dsListRoles(dsHandle, out rolesPtr);
			if (result == 0) {
				try {
