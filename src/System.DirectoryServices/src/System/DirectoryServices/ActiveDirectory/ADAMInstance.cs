//------------------------------------------------------------------------------
// <copyright file="ADAMInstance.cs" company="Microsoft">
...
namespace System.DirectoryServices.ActiveDirectory {
	...
	{
		private string[] becomeRoleOwnerAttrs = null; 
		...
		private bool defaultPartitionInitialized = false;
		private bool defaultPartitionModified = false;
		private ConfigurationSet currentConfigSet = null;
		private string cachedDefaultPartition = null;
		private AdamRoleCollection cachedRoles = null;
		
		private IntPtr ADAMHandle = (IntPtr)0;
		private IntPtr authIdentity = IntPtr.Zero;
		private SyncUpdateCallback userDelegate = null;              
		private SyncReplicaFromAllServersCallback syncAllFunctionPointer = null;
		...
		public void TransferRoleOwnership(AdamRole role) {
			...
			try {
				DirectoryEntry rootDSE = directoryEntryMgr.GetCachedDirectoryEntry(WellKnownDN.RootDSE);
				rootDSE.Properties[becomeRoleOwnerAttrs[(int)role]].Value = 1;
		...
		public override ReplicationCursorCollection GetReplicationCursors(string partition) {
			...
			// get the handle
			GetADAMHandle();
			info = GetReplicationInfoHelper(ADAMHandle, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_CURSORS_3_FOR_NC, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_CURSORS_FOR_NC, partition, ref advanced, context, DirectoryContext.ADAMHandle);
			return ConstructReplicationCursors(ADAMHandle, advanced, info, partition, this, DirectoryContext.ADAMHandle);
		}
		...
		public override ReplicationOperationInformation GetReplicationOperationInformation() {
			...
			// get the handle
			GetADAMHandle();
			info = GetReplicationInfoHelper(ADAMHandle, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_PENDING_OPS, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_PENDING_OPS, null, ref advanced, 0, DirectoryContext.ADAMHandle);
		...
		public override ReplicationNeighborCollection GetReplicationNeighbors(string partition) {
			...
			// get the handle
			GetADAMHandle();
			info = GetReplicationInfoHelper(ADAMHandle, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_NEIGHBORS, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_NEIGHBORS, partition, ref advanced, 0, DirectoryContext.ADAMHandle);
		...
		public override ReplicationNeighborCollection GetAllReplicationNeighbors() {
			...
			// get the handle
			GetADAMHandle();
			info = GetReplicationInfoHelper(ADAMHandle, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_NEIGHBORS, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_NEIGHBORS, null, ref advanced, 0, DirectoryContext.ADAMHandle);
			return ConstructNeighbors(info, this, DirectoryContext.ADAMHandle);
		}
		...
		public override ActiveDirectoryReplicationMetadata GetReplicationMetadata(string objectPath) {
			...
			// get the handle
			GetADAMHandle();
			info = GetReplicationInfoHelper(ADAMHandle, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_METADATA_2_FOR_OBJ, (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_METADATA_FOR_OBJ, objectPath, ref advanced, 0, DirectoryContext.ADAMHandle);
		...
		public override void SyncReplicaFromServer(string partition, string sourceServer) {
			...
			// get the dsHandle
			GetADAMHandle();
			SyncReplicaHelper(ADAMHandle, true, partition, sourceServer, 0, DirectoryContext.ADAMHandle);
		}
		...
		public override void TriggerSyncReplicaFromNeighbors(string partition) {
			...
			// get the dsHandle
			GetADAMHandle();
			SyncReplicaHelper(ADAMHandle, true, partition, null, DS_REPSYNC_ASYNCHRONOUS_OPERATION | DS_REPSYNC_ALL_SOURCES, DirectoryContext.ADAMHandle);
		}
		...
		public ConfigurationSet ConfigurationSet {
			get {
				...
				if (currentConfigSet == null) {
					...
					currentConfigSet = ConfigurationSet.GetConfigurationSet(configSetContext);
				}
		...
		public string HostName {
			get {
				...
				if (cachedHostName == null) {
					...
					cachedHostName = (string) PropertyManager.GetPropertyValue(context, serverEntry, PropertyManager.DnsHostName);		
				}
		...
		public int LdapPort {
			get {
				...
				if (cachedLdapPort == -1) {
					DirectoryEntry ntdsaEntry = directoryEntryMgr.GetCachedDirectoryEntry(NtdsaObjectName);
					cachedLdapPort = (int) PropertyManager.GetPropertyValue(context, ntdsaEntry, PropertyManager.MsDSPortLDAP);
				}
		...
		public int SslPort {
			get {
				...
				if (cachedSslPort == -1) {
					DirectoryEntry ntdsaEntry = directoryEntryMgr.GetCachedDirectoryEntry(NtdsaObjectName);
					cachedSslPort = (int) PropertyManager.GetPropertyValue(context, ntdsaEntry, PropertyManager.MsDSPortSSL);
				}
			...
			get{
				...
				try {
					if (cachedRoles == null) {
						...
						cachedRoles = new AdamRoleCollection(roleList);
					}
				}
				catch (COMException e) {
					throw ExceptionHelper.GetExceptionFromCOMException(context, e);
				...
				if (!defaultPartitionInitialized || defaultPartitionModified) {
					...
					try {
						...
						if (ntdsaEntry.Properties[PropertyManager.MsDSDefaultNamingContext].Value == null) {
							...
							cachedDefaultPartition = (string)PropertyManager.GetPropertyValue(context, ntdsaEntry, PropertyManager.MsDSDefaultNamingContext);
						}
					}
					catch (COMException e) {
						throw ExceptionHelper.GetExceptionFromCOMException(context, e);
		...
		private ReplicationFailureCollection GetReplicationFailures(DS_REPL_INFO_TYPE type) {
			...
			// get the handle
			GetADAMHandle();
			info = GetReplicationInfoHelper(ADAMHandle, (int)type, (int)type, null, ref advanced, 0, DirectoryContext.ADAMHandle);
		...
		private void GetADAMHandle() {
            ...
            lock(this) {
				if (ADAMHandle == IntPtr.Zero) {
					...
					ADAMHandle = Utils.GetDSHandle(bindingString, null, authIdentity, DirectoryContext.ADAMHandle);
				}
			}
		}
