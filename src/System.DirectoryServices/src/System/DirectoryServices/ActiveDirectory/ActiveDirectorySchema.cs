//------------------------------------------------------------------------------
// <copyright file="ActiveDirectorySchema.cs" company="Microsoft">
...
namespace System.DirectoryServices.ActiveDirectory {
	...
	public class ActiveDirectorySchema: ActiveDirectoryPartition {
		...
		private DirectoryEntry abstractSchemaEntry = null;
             	private DirectoryServer cachedSchemaRoleOwner = null;
		...
		public ActiveDirectorySchemaClass FindDefunctClass(string commonName) {
			...
			Hashtable propertiesFromServer = ActiveDirectorySchemaClass.GetPropertiesFromSchemaContainer(context, schemaEntry, commonName, true /* isDefunctOnServer */);
			ActiveDirectorySchemaClass schemaClass = new ActiveDirectorySchemaClass(context, commonName, propertiesFromServer, schemaEntry);
		...
		public ActiveDirectorySchemaProperty FindDefunctProperty(string commonName) {
			...
			SearchResult propertiesFromServer = ActiveDirectorySchemaProperty.GetPropertiesFromSchemaContainer(context, schemaEntry, commonName, true /* isDefunctOnServer */);
			ActiveDirectorySchemaProperty schemaProperty = new ActiveDirectorySchemaProperty(context, commonName, propertiesFromServer, schemaEntry);
		...
		public ReadOnlyActiveDirectorySchemaPropertyCollection FindAllProperties(PropertyTypes type) {
			...
			return GetAllProperties(context, schemaEntry, str.ToString());
		}
		...
		private DirectoryServer GetSchemaRoleOwner() {
			...
			try {
				...
				if (context.isADAMConfigSet()) {
					// ADAM
					string adamInstName = Utils.GetAdamDnsHostNameFromNTDSA(context, (string)PropertyManager.GetPropertyValue(context, schemaEntry, PropertyManager.FsmoRoleOwner));
					...
					if (Utils.CheckCapability(rootDSE, Capability.ActiveDirectory)) {
						string dcName = Utils.GetDnsHostNameFromNTDSA(context, (string)PropertyManager.GetPropertyValue(context, schemaEntry, PropertyManager.FsmoRoleOwner));
