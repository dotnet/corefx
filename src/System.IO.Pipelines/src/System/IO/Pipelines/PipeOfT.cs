...
namespace System.IO.Pipelines
{
    ...
    public sealed partial class Pipe<T>
    {
        ...
        private static readonly Action<object> s_signalReaderAwaitable = state => ((Pipe<T>)state).ReaderCancellationRequested();
        private static readonly Action<object> s_signalWriterAwaitable = state => ((Pipe<T>)state).WriterCancellationRequested();
        ...
        private readonly BufferSegment<T>[] _bufferSegmentPool;
        ...
        /// <summary>
        /// Initializes the <see cref="Pipe{T}"/> using <see cref="PipeOptions{T}.Default"/> as options.
        /// </summary>
        public Pipe() : this(PipeOptions<T>.Default)
        {
        }

        /// <summary>
        /// Initializes the <see cref="Pipe{T}"/> with the specified <see cref="PipeOptions{T}"/>.
        /// </summary>
        public Pipe(PipeOptions<T> options)
        {
            ...
            _bufferSegmentPool = new BufferSegment<T>[SegmentPoolSize];
        ...
        internal Memory<T> GetMemory(int sizeHint)
        {
            ...
            Memory<T> availableMemory = _writingHead.AvailableMemory;
            availableMemory = availableMemory.Slice(end);
            return availableMemory;
        }

        internal Span<T> GetSpan(int sizeHint)
        {
            if (_writerCompletion.IsCompleted)
            {
                ThrowHelper.ThrowInvalidOperationException_NoWritingAllowed();
            ...
            // Slice the AvailableMemory to the WritableBytes size
            int end = _writingHead.End;
            Span<T> availableSpan = _writingHead.AvailableMemory.Span;
        ...
        internal void Advance(int bytesWritten)
        {
            ...
            if (bytesWritten >= 0)
            {
                ...
                Memory<T> buffer = _writingHead.AvailableMemory;
        ...
        internal void AdvanceReader(in SequencePosition consumed, in SequencePosition examined)
        {
            ...
            AdvanceReader((BufferSegment<T>)consumed.GetObject(), consumed.GetInteger(), (BufferSegment<T>)examined.GetObject(), examined.GetInteger());
        }

        private void AdvanceReader(BufferSegment<T> consumedSegment, int consumedIndex, BufferSegment<T> examinedSegment, int examinedIndex)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static long GetLength(BufferSegment<T> startSegment, int startIndex, BufferSegment<T> endSegment, int endIndex)
        {
        ...
        internal ValueTask<ReadResult<T>> ReadAsync(CancellationToken token)
        {
            if (_readerCompletion.IsCompleted)
            {
                ThrowHelper.ThrowInvalidOperationException_NoReadingAllowed();
            ...
            lock (_sync)
            {
                ...
                {
                    GetReadResult(out ReadResult<T> readResult);
                    result = new ValueTask<ReadResult<T>>(readResult);
                }
                else
                {
                    // Otherwise it's async
                    result = new ValueTask<ReadResult<T>>(_reader, token: 0);
                }
            }
        ...
        internal bool TryRead(out ReadResult<T> result)
        {
            lock (_sync)
            {
                if (_readerCompletion.IsCompleted)
        ...
        internal ReadResult<T> GetReadAsyncResult()
        {
            ReadResult<T> result;
        ...
        private void GetReadResult(out ReadResult<T> result)
        {
            ...
            if (head != null)
            {
                ...
                result = new ReadResult<T>(readOnlySequence, isCanceled, isCompleted);
            }
            else
            {
                result = new ReadResult<T>(default, isCanceled, isCompleted);
            }
        ...
        /// </summary>
        public PipeReader<T> Reader => _reader;

        /// <summary>
        /// Gets the <see cref="PipeWriter"/> for this pipe.
        /// </summary>
        public PipeWriter<T> Writer => _writer;
