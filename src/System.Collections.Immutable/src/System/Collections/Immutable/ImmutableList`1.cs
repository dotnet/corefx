...
namespace System.Collections.Immutable
{
    ...
    public sealed partial class ImmutableList<T> : IImmutableList<T>, IList<T>, IList, IOrderedCollection<T>, IImmutableListQueries<T>
    {
        ...
        public ImmutableList<T> Add(T value)
        {
            ...
            var result = this._root.Add(value);
        ...
        public ImmutableList<T> AddRange(IEnumerable<T> items)
        {
            ...
            var result = this._root.AddRange(items);
        ...
        public ImmutableList<T> InsertRange(int index, IEnumerable<T> items)
        {
            ...
            var result = this._root.InsertRange(index, items);
        ...
        public ImmutableList<T> RemoveAt(int index)
        {
            ...
            Contract.Ensures(Contract.Result<ImmutableList<T>>() != null);
            Contract.Ensures(Contract.Result<ImmutableList<T>>().Count == this.Count - 1);
            var result = this._root.RemoveAt(index);
            return this.Wrap(result);
        }
        ...
        public ImmutableList<TOutput> ConvertAll<TOutput>(Func<T, TOutput> converter)
        {
            ...
            return ImmutableList<TOutput>.WrapNode(this._root.ConvertAll(converter));
        }
        ...
        public struct Enumerator : IEnumerator<T>, ISecurePooledObjectUser
        {
            ...
            private void ResetStack()
            {
                ...
                var node = this._root;
                var skipNodes = this._reversed ? this._root.Count - this._startIndex - 1 : this._startIndex;
            ...
            {
                return this._reversed ? node.Left : node.Right;
            }
            ...
            {
                return this._reversed ? node.Right : node.Left;
            }
        ...
        internal sealed class Node : IBinaryTree<T>, IEnumerable<T>
        {
            ...
            public bool IsEmpty
            {
                get
                {
                    Contract.Ensures(Contract.Result<bool>() == (this._left == null));
            ...
            public int Count
            {
                get
                {
                    Contract.Ensures(Contract.Result<int>() == this._count);
                    return this._count;
                }
            }
            ...
            internal Node Insert(int index, T key)
            {
                ...
                else
                {
                    ...
                    if (index <= this._left._count)
                    {
                        var newLeft = this._left.Insert(index, key);
                        result = this.Mutate(left: newLeft);
                    }
                    else
                    {
                        var newRight = this._right.Insert(index - this._left._count - 1, key);
            ...
            internal Node InsertRange(int index, IEnumerable<T> keys)
            {
                ...
                else
                {
                    ...
                    if (index <= this._left._count)
                    {
                        var newLeft = this._left.InsertRange(index, keys);
                        result = this.Mutate(left: newLeft);
                    }
                    else
                    {
                        var newRight = this._right.InsertRange(index - this._left._count - 1, keys);
                        result = this.Mutate(right: newRight);
                    }
            ...
            internal Node RemoveAt(int index)
            {
                ...
                if (index == this._left._count)
                {
                    ...
                    else
                    {
                        ...
                        var newRight = this._right.RemoveAt(0);
                        result = successor.Mutate(left: this._left, right: newRight);
                    }
                }
                else if (index < this._left._count)
                {
                    var newLeft = this._left.RemoveAt(index);
                    result = this.Mutate(left: newLeft);
                }
                else
                {
                    var newRight = this._right.RemoveAt(index - this._left._count - 1);
                    result = this.Mutate(right: newRight);
                }
            ...
            internal Node ReplaceAt(int index, T value)
            {
                ...
                {
                    var newLeft = this._left.ReplaceAt(index, value);
                    result = this.Mutate(left: newLeft);
                }
                else
                {
                    var newRight = this._right.ReplaceAt(index - this._left._count - 1, value);
                    result = this.Mutate(right: newRight);
                }
            ...
            internal int BinarySearch(int index, int count, T item, IComparer<T> comparer)
            {
                ...
                int thisNodeIndex = this._left.Count; // this is only the index within the AVL tree, treating this node as root rather than a member of a larger tree.
                ...
                {
                    int result = this._right.BinarySearch(index - thisNodeIndex - 1, count, item, comparer);
                ...
                int compare = comparer.Compare(item, this._key);
                if (compare == 0)
                {
                ...
                else if (compare > 0)
                {
                    ...
                    int result = adjustedCount < 0 ? -1 : this._right.BinarySearch(0, adjustedCount, item, comparer);
                    int offset = thisNodeIndex + 1;
                    return result < 0 ? result - offset : result + offset;
                }
                else
                {
                    ...
                    int result = this._left.BinarySearch(index, count, item, comparer);
            ...
            private Node Mutate(T value)
            {
                if (this._frozen)
                {
                    return new Node(value, this._left, this._right);
                }
                else
                {
                    this._key = value;
