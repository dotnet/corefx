...
namespace System.Collections.Immutable
{
    ...
    public sealed partial class ImmutableSortedSet<T> : IImmutableSet<T>, ISortKeyCollection<T>, IReadOnlyList<T>, IList<T>, ISet<T>, IList
    {
        ...
        public ImmutableSortedSet<T> Clear()
        {
            ...
            return this._root.IsEmpty ? this : Empty.WithComparer(this._comparer);
        }
        ...
        public ImmutableSortedSet<T> Add(T value)
        {
            ...
            Contract.Ensures(Contract.Result<ImmutableSortedSet<T>>() != null);
            bool mutated;
            return this.Wrap(this._root.Add(value, this._comparer, out mutated));
        }
        ...
        public ImmutableSortedSet<T> Remove(T value)
        {
            ...
            Contract.Ensures(Contract.Result<ImmutableSortedSet<T>>() != null);
            bool mutated;
            return this.Wrap(this._root.Remove(value, this._comparer, out mutated));
        }
        ...
        public bool TryGetValue(T equalValue, out T actualValue)
        {
            ...
            Node searchResult = this._root.Search(equalValue, this._comparer);
        ...
        public ImmutableSortedSet<T> Except(IEnumerable<T> other)
        {
            ...
            {
                bool mutated;
                result = result.Remove(item, this._comparer, out mutated);
            }
        ...
        {
            return new ReverseEnumerable(this._root);
        }
        ...
        private ImmutableSortedSet<T> UnionIncremental(IEnumerable<T> items)
        {
            ...
            {
                bool mutated;
                result = result.Add(item, this._comparer, out mutated);
            }
        ...
        private ImmutableSortedSet<T> Wrap(Node root)
        {
            if (root != this._root)
            {
                return root.IsEmpty ? this.Clear() : new ImmutableSortedSet<T>(root, this._comparer);
            }
            else
            {
                return this;
        ...
        public struct Enumerator : IEnumerator<T>, ISecurePooledObjectUser
        {
            ...
            /// </remarks>
            private static readonly SecureObjectPool<Stack<RefAsValueType<Node>>, Enumerator> _enumeratingStacks =
            ...
            internal Enumerator(Node root, Builder builder = null, bool reverse = false)
            {
                ...
                this._stack = null;
                if (!_enumeratingStacks.TryTake(this, out this._stack))
                {
                    this._stack = _enumeratingStacks.PrepNew(this, new Stack<RefAsValueType<Node>>(root.Height));
                }
            ...
            public void Dispose()
            {
                ...
                if (this._stack != null && this._stack.TryUse(ref this, out stack))
                {
                    stack.ClearFastWhenEmpty();
                    _enumeratingStacks.TryAdd(this, this._stack);
                    this._stack = null;
                }
            }
            ...
            public bool MoveNext()
            {
                ...
                var stack = this._stack.Use(ref this);
                if (stack.Count > 0)
                {
                    Node n = stack.Pop().Value;
                    this._current = n;
                    this.PushNext(this._reverse ? n.Left : n.Right);
                    return true;
                }
                else
                {
            ...
            {
                Contract.Ensures(this._root != null);
                Contract.EnsuresOnThrow<ObjectDisposedException>(this._root == null);
                ...
                if (this._root == null || (this._stack != null && !this._stack.IsOwned(ref this)))
                {
        ...
        internal sealed class Node : IBinaryTree<T>, IEnumerable<T>
        {
            ...
            private Node()
            {
                ...
                this._frozen = true; // the empty node is *always* frozen.
            }
            ...
            /// </summary>
            public T Value { get { return this._key; } }
            ...
            internal Node Add(T key, IComparer<T> comparer, out bool mutated)
            {
                ...
                {
                    Node result = this;
                    int compareResult = comparer.Compare(key, this._key);
                    if (compareResult > 0)
                    {
                        var newRight = this._right.Add(key, comparer, out mutated);
                        if (mutated)
                        {
                    ...
                    {
                        var newLeft = this._left.Add(key, comparer, out mutated);
                        if (mutated)
                        {
            ...
            internal Node Remove(T key, IComparer<T> comparer, out bool mutated)
            {
                ...
                {
                    Node result = this;
                    int compare = comparer.Compare(key, this._key);
                    if (compare == 0)
                    {
                        // We have a match.
                        mutated = true;
                        ...
                        else
                        {
                            ...
                            bool dummyMutated;
                            var newRight = this._right.Remove(successor._key, comparer, out dummyMutated);
                            result = successor.Mutate(left: this._left, right: newRight);
                        }
                    }
                    else if (compare < 0)
                    {
                        var newLeft = this._left.Remove(key, comparer, out mutated);
                        if (mutated)
                        {
                            result = this.Mutate(left: newLeft);
                        }
                    ...
                    {
                        var newRight = this._right.Remove(key, comparer, out mutated);
                        if (mutated)
                        {
                            result = this.Mutate(right: newRight);
                        }
            ...
            internal int IndexOf(T key, IComparer<T> comparer)
            {
                ...
                else
                {
                    ...
                    {
                        int result = this._right.IndexOf(key, comparer);
                        ...
                        result = this._left.Count + 1 + result;
                        if (missing)
                        {
                            result = ~result;
                        }
            ...
            private Node Mutate(Node left = null, Node right = null)
            {
                if (this._frozen)
                {
                    return new Node(this._key, left ?? this._left, right ?? this._right);
                }
                else
                {
                    if (left != null)
