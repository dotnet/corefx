...
namespace System.Reflection.Metadata
{
    ...
    public unsafe struct BlobReader
    {
        ...
        public int Offset
        {
            get
            {
                return (int)(this._currentPointer - _block.Pointer);
            }
        }
        ...
        public int RemainingBytes
        {
            get
            {
                return (int)(this._endPointer - this._currentPointer);
            }
        }

        public void Reset()
        {
            this._currentPointer = _block.Pointer;
        }
        ...
        internal bool SeekOffset(int offset)
        {
            ...
            this._currentPointer = _block.Pointer + offset;
            return true;
        }
        ...
        {
            if (unchecked((ulong)(uint)offset + (uint)byteCount) > (ulong)(this._endPointer - this._currentPointer))
            {
                ThrowOutOfBounds();
            }
        }
        ...
        {
            if (unchecked((uint)byteCount) > (this._endPointer - this._currentPointer))
            {
                ThrowOutOfBounds();
            }
        }
        ...
        {
            byte* p = this._currentPointer;

            if (unchecked((uint)length) > (uint)(this._endPointer - p))
            {
                ThrowOutOfBounds();
            }

            this._currentPointer = p + length;
            return p;
        }
        ...
        private byte* GetCurrentPointerAndAdvance1()
        {
            ...
            if (p == this._endPointer)
            {
                ThrowOutOfBounds();
            }

            this._currentPointer = p + 1;
            return p;
        }
        ...
        {
            string s = this._block.PeekUtf8(this.Offset, byteCount);
        ...
        {
            string s = this._block.PeekUtf16(this.Offset, byteCount);
            this._currentPointer += byteCount;
            return s;
        }
        ...
        {
            byte[] bytes = this._block.PeekBytes(this.Offset, byteCount);
            this._currentPointer += byteCount;
            return bytes;
        }
        ...
        {
            int bytesRead;
            string value = this._block.PeekUtf8NullTerminated(this.Offset, null, MetadataStringDecoder.DefaultUTF8, out bytesRead, '\0');
        ...
        {
            int bytesRead;
            int value = this._block.PeekCompressedInteger(this.Offset, out bytesRead);
            this._currentPointer += bytesRead;
            return value;
        }
        ...
        {
            int bytesRead;
            value = this._block.PeekCompressedInteger(this.Offset, out bytesRead);
        ...
        public Handle ReadTypeHandle()
        {
            ...
            uint tokenType = _corEncodeTokenArray[value & 0x3];
        ...
        private static readonly uint[] _corEncodeTokenArray = new uint[] { TokenTypeIds.TypeDef, TokenTypeIds.TypeRef, TokenTypeIds.TypeSpec, 0 };
        #endregion
    }
}
