...
namespace System.Reflection.Metadata
{
    partial class MetadataReader
    {
        ...
        private static ProjectionInfo[] _projectionInfos;
        ...
        private TypeDefTreatment GetWellKnownTypeDefinitionTreatment(TypeDefinitionHandle typeDef)
        {
            ...
            if (StringStream.EqualsRaw(namespaceName, StringStream.GetVirtualValue(_projectionInfos[index].ClrNamespace)))
            {
                return _projectionInfos[index].Treatment;
            }

            // TODO: we can avoid this comparison if info.DotNetNamespace == info.WinRtNamespace 
            if (StringStream.EqualsRaw(namespaceName, _projectionInfos[index].WinRTNamespace))
            {
                return _projectionInfos[index].Treatment | TypeDefTreatment.MarkInternalFlag;
            }
        ...
        private int GetProjectionIndexForTypeReference(TypeReferenceHandle typeRef, out bool isIDisposable)
        {
            ...
            if (index >= 0 && StringStream.EqualsRaw(TypeRefTable.GetNamespace(typeRef), _projectionInfos[index].WinRTNamespace))
            {
                isIDisposable = _projectionInfos[index].IsIDisposable;
                return index;
            }
        ...
        {
            Debug.Assert(_projectionInfos != null && projectionIndex >= 0 && projectionIndex < _projectionInfos.Length);
            return AssemblyReferenceHandle.FromVirtualIndex(_projectionInfos[projectionIndex].AssemblyRef);
        }

        internal static StringHandle GetProjectedName(int projectionIndex)
        {
            Debug.Assert(_projectionInfos != null && projectionIndex >= 0 && projectionIndex < _projectionInfos.Length);
            return StringHandle.FromVirtualIndex(_projectionInfos[projectionIndex].ClrName);
        }

        internal static StringHandle GetProjectedNamespace(int projectionIndex)
        {
            Debug.Assert(_projectionInfos != null && projectionIndex >= 0 && projectionIndex < _projectionInfos.Length);
            return StringHandle.FromVirtualIndex(_projectionInfos[projectionIndex].ClrNamespace);
        }

        private static void InitializeProjectedTypes()
        {
            if (_projectedTypeNames == null || _projectionInfos == null)
            {
