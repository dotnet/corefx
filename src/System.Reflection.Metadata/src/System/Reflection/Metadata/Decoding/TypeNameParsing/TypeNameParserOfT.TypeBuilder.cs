    ...
    internal partial class TypeNameParser<TType>
    {
        private class TypeBuilder
        {
            ...
            public TType BuildType(ITypeNameParserTypeProvider<TType> typeProvider)
            {
                ...
                type = BuildGenericInstantiationIfNeeded(typeProvider, type);
            ...
            private TType BuildGenericInstantiationIfNeeded(ITypeNameParserTypeProvider<TType> typeProvider, TType genericType)
            {
                if (_genericTypeArguments == null)
                    return genericType;

                Debug.Assert(_genericTypeArguments.Count > 0);
                return typeProvider.GetGenericInstantiation(genericType, _genericTypeArguments.ToImmutableArray());
            }
