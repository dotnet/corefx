...
namespace System.Reflection.Metadata.Ecma335
{
    ...
    {
        private static string[] _virtualValues;

        internal readonly MemoryBlock Block;

        internal StringStreamReader(MemoryBlock block, MetadataKind metadataKind)
        {
            if (_virtualValues == null && metadataKind != MetadataKind.Ecma335)
            {
                // Note:
        ...
        {
            for (int i = 0; i < _virtualValues.Length; i++)
            {
                Debug.Assert(_virtualValues[i] != null, "Missing virtual value for StringHandle.VirtualIndex." + (StringHandle.VirtualIndex)i);
            }
        }
        ...
        {
            return _virtualValues[(int)index];
        }
    ...
    internal unsafe struct BlobStreamReader
    {
        ...
        private static byte[][] _virtualHeapBlobs;
        ...
        internal BlobStreamReader(MemoryBlock block, MetadataKind metadataKind)
        {
            ...
            if (_virtualHeapBlobs == null && metadataKind != MetadataKind.Ecma335)
            {
        ...
        internal BlobReader GetBlobReader(BlobHandle handle)
        {
            if (handle.IsVirtual)
            {
                ...
                int length = _virtualHeapBlobs[index].Length;
        ...
        internal byte[] GetVirtualBlobArray(BlobHandle handle, bool unique)
        {
            ...
            byte[] result = _virtualHeapBlobs[(int)index];
