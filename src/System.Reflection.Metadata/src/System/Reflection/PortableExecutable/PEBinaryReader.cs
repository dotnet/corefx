...
namespace System.Reflection.PortableExecutable
{
    ...
    internal struct PEBinaryReader
    {
        ...
        public PEBinaryReader(Stream stream, int size)
        {
            ...
            this._maxOffset = this._startOffset + (uint)size;
            this._reader = new BinaryReader(stream, Encoding.UTF8, leaveOpen: true);
        }
        ...
        private void CheckBounds(uint count)
        {
            ...
            Debug.Assert(_reader.BaseStream.Position >= 0 && this._maxOffset >= 0);

            // Add cannot overflow because the worst case is (ulong)long.MaxValue + uint.MaxValue < ulong.MaxValue.
            if ((ulong)_reader.BaseStream.Position + count > (ulong)this._maxOffset)
            {
                ThrowImageTooSmall();
            }
        }
        ...
        {
            Debug.Assert(startPosition >= 0 && this._maxOffset >= 0);

            // Add cannot overflow because the worst case is (ulong)long.MaxValue + uint.MaxValue < ulong.MaxValue.
            // Negative count is handled by overflow to greater than maximum size = int.MaxValue.
            if ((ulong)startPosition + unchecked((uint)count) > (ulong)this._maxOffset)
            {
