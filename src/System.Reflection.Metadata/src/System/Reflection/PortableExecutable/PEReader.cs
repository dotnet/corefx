    ...
    public sealed class PEReader : IDisposable
    {
        ...
        private unsafe PEReader(Stream peStream, PEStreamOptions options, int? sizeOpt)
        {
            ...
            try
            {
                ...
                {
                    this._peImage = new StreamMemoryBlockProvider(peStream, start, size, isFileStream, (options & PEStreamOptions.LeaveOpen) != 0);
                    closeStream = false;
                }
                else
                {
                    ...
                    if ((options & PEStreamOptions.PrefetchEntireImage) != 0)
                    {
                        ...
                        this._lazyImageBlock = imageBlock;
                        this._peImage = new ExternalMemoryBlockProvider(imageBlock.Pointer, imageBlock.Size);
                    ...
                    else
                    {
                        ...
                        this._lazyMetadataBlock = StreamMemoryBlockProvider.ReadMemoryBlockNoLock(peStream, isFileStream, _lazyPEHeaders.MetadataStartOffset, _lazyPEHeaders.MetadataSize);
                    }
