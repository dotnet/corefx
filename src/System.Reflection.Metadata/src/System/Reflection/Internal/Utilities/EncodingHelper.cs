...
namespace System.Reflection.Internal
{
    ...
    internal unsafe static class EncodingHelper
    {
        ...
        private static Encoding_GetString _getStringPlatform = LoadGetStringPlatform(); // only non-readonly for test hook

        public static string GetString(this Encoding encoding, byte* bytes, int byteCount)
        {
            Debug.Assert(encoding != null);

            if (_getStringPlatform == null)
            {
                return GetStringPortable(encoding, bytes, byteCount);
            }

            return _getStringPlatform(encoding, bytes, byteCount);
        }
        ...
        {
            get { return _getStringPlatform != null; }
            set { _getStringPlatform = value ? LoadGetStringPlatform() : null; }
        }
        #endregion
    }
}
