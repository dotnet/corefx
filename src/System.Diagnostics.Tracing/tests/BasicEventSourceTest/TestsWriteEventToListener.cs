...
#if false
// Only run "Write" tests for M.D.T.
...
namespace BasicEventSourceTests
{
    ...
    public class TestsWriteEventToListener
    {
        ...
        public void Test_WriteEvent_InvalidCalls()
        {
            ...
            {
                TestUtilities.CheckNoEventSourcesRunning("Start");
                // disable until we figure out how we fix this in BCL
#if FALSE
            var log = new InvalidCallsToWriteEventEventSource();
            using (var el = new LoudListener())
            {
                log.WriteTooManyArgs("Hello");
                Assert.Equal(2, LoudListener.LastEvent.EventId);
                Assert.Equal(2, LoudListener.LastEvent.Payload.Count);           // Faked count (compat)
                Assert.Equal("Hello", LoudListener.LastEvent.Payload[0]);
                Assert.Equal(string.Empty, LoudListener.LastEvent.Payload[1]);   // Faked argument (compat)
            ...
            lock (TestUtilities.EventSourceTestLock)
            {
                ...
                using (var log = new EventSourceTest())
                {
                    using (var el = new LoudListener())
                    {
                        ...
                        actid = Guid.NewGuid();
                        ExceptionAssert.Throws<EventSourceException>(
                            () => log.LogTransferNoOpcode(actid));
                    }
                }
                TestUtilities.CheckNoEventSourcesRunning("Stop");
            }
...
#if false // TODO: EventListener events are not enabled yet.
        [Fact]
