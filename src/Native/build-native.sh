...
usage()
{
    echo "Usage: $0 [BuildArch] [BuildType] [verbose] [clangx.y] [platform] [cross] [staticLibLink] [cmakeargs] [makeargs]"
...
prepare_native_build()
{
    ...
    if [ $__generateversionsource == true ]; then
        $__rootRepo/init-tools.sh
    fi

    # Generate version.c if specified, else have an empty one.
    __versionSourceFile=$__rootbinpath/obj/version.c
    if [ ! -e "${__versionSourceFile}" ]; then
        if [ $__generateversionsource == true ]; then
            $__rootRepo/Tools/dotnetcli/dotnet $__rootRepo/Tools/MSBuild.exe "$__rootRepo/build.proj" /t:GenerateVersionSourceFile /p:GenerateVersionSourceFile=true /v:minimal
        else
