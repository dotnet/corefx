...
AppleCryptoNative_X509DemuxAndRetainHandle(CFTypeRef handle, SecCertificateRef* pCertOut, SecIdentityRef* pIdentityOut)
{
    ...
    {
        *pIdentityOut = (SecIdentityRef)handle;
    }
    else if (objectType == SecCertificateGetTypeID())
    {
        *pCertOut = (SecCertificateRef)handle;
    }
    else
    {
        return 0;
...
static int32_t ProcessCertificateTypeReturn(CFArrayRef items, SecCertificateRef* pCertOut, SecIdentityRef* pIdentityOut)
{
    ...
    {
        CFRetain(bestItem);
        *pCertOut = (SecCertificateRef)bestItem;
        return 1;
    }
    ...
    {
        CFRetain(bestItem);
        *pIdentityOut = (SecIdentityRef)bestItem;
                                                 ...
                                                 int32_t* pOSStatus)
{
    ...
    if (status == noErr)
    {
        searchList = CFArrayCreate(
            NULL, (const void**)&targetKeychain, 1, &kCFTypeArrayCallBacks);
    ...
    if (status == noErr)
    {
        itemMatch = CFArrayCreate(
            NULL, (const void**)&cert, 1, &kCFTypeArrayCallBacks);
