...
namespace System.SpanTests
{
    public static partial class ReadOnlySpanTests
    {
        ...
        {
            public string TestFilePath => GetTestFilePath(0, "ReadOnlySpanTests_CopyToLargeSizeTest");
        }
            ...
            if (AllocationHelper.TryAllocNative((IntPtr)1, out IntPtr pMemory))  // Synchronize with other big-memory tests to reduce stress on machine. The memory itself isn't used.
            {
                try
                {
                    ...
                    using (CopyToLargeSizeTestFileCleanup fileCleanup = new CopyToLargeSizeTestFileCleanup())
                    {
                        ...
                        try
                        {
                            long capacity = checked(bufferSize * 2);
                            using (MemoryMappedFile memoryMappedFile = MemoryMappedFile.CreateFromFile(testFilePath, FileMode.Create, null, capacity, MemoryMappedFileAccess.ReadWrite))
                            ...
                            using (SafeHandle viewAccessorSafeHandle = viewAccessor.SafeMemoryMappedViewHandle)
                            {
                                unsafe
                                {
                                    ...
                                    var spanFirst = new ReadOnlySpan<Guid>(memBlockFirst, GuidCount);
