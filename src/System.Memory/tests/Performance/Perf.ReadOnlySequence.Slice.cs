        ...
        private static void Byte_Empty()
        {
            ...
            foreach (BenchmarkIteration iteration in Benchmark.Iterations)
            {
                ...
                using (iteration.StartMeasurement())
                {
                    for (int i = 0; i < Benchmark.InnerIterationCount; i++)
                    {
                        ReadOnlySequence<byte> sliced = buffer.Slice(0L);
                        localInt ^= sliced.Start.GetInteger();

                        sliced = sliced.Slice(0L, 0L);
                        localInt ^= sliced.Start.GetInteger();

                        sliced = sliced.Slice(0L, sliced.End);
                        localInt ^= sliced.Start.GetInteger();

                        sliced = sliced.Slice(sliced.Start);
                        localInt ^= sliced.Start.GetInteger();

                        sliced = sliced.Slice(sliced.Start, 0L);
                        localInt ^= sliced.Start.GetInteger();

                        sliced = sliced.Slice(sliced.Start, sliced.End);
                        localInt ^= sliced.Start.GetInteger();

                        sliced = sliced.Slice(0, 0);
                        localInt ^= sliced.Start.GetInteger();

                        sliced = sliced.Slice(0, sliced.End);
                        localInt ^= sliced.Start.GetInteger();

                        sliced = sliced.Slice(sliced.Start, 0);
                        localInt ^= sliced.Start.GetInteger();
                    }
                }
                _volatileInt = localInt;
