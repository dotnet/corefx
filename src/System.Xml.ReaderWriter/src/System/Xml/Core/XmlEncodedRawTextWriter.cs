...
namespace System.Xml
{
    ...
    internal partial class XmlEncodedRawTextWriter : XmlRawWriter
    {
        ...
        protected bool mergeCDataSections = false;
        protected bool autoXmlDeclaration = false;
        ...
        protected virtual void FlushBuffer()
        {
            ...
            {
                // Future calls to flush (i.e. when Dispose() is called) don't attempt to write to stream
                writeToNull = true;
                throw;
            }
            finally
        ...
        private static unsafe char* EncodeSurrogate(char* pSrc, char* pSrcEnd, char* pDst)
        {
            ...
            if (ch <= XmlCharType.SurHighEnd)
            {
                ...
                {
                    int lowChar = pSrc[1];
                    if (lowChar >= XmlCharType.SurLowStart && lowChar <= XmlCharType.SurLowEnd)
                    {
                        pDst[0] = (char)ch;
                        pDst[1] = (char)lowChar;
                        pDst += 2;
        ...
        protected unsafe void ValidateContentChars(string chars, string propertyName, bool allowOnlyWhitespace)
        {
            ...
            Error:
                throw new ArgumentException(SR.Format(SR.Xml_InvalidCharsInIndent, propertyName, error));
            }
        }
    }
