...
namespace System.Reflection.Tests
{
    ...
    {
        // Given a runtime Type, load up the equivalent in the Test typeloader. This is for test-writing convenience so 
        ...
        public static Type Project(this Type type)
        {
            ...
            Assembly projectedAssembly = s_assemblyDict.GetOrAdd(assembly,
                delegate (Assembly a) 
                {
                    // The core assembly we're using might not be the one powering the runtime. Make sure we project to the core assembly the typeloader
                    // is using.
                    if (a == typeof(object).Assembly)
                        return TestTypeLoader.LoadFromStream(CreateStreamForCoreAssembly());

                    return TestTypeLoader.LoadFromAssemblyPath(a.Location);
                });
