...
namespace System.Reflection.Tests
{
    internal static partial class TestUtils
    {
        ...
        // For the tests that exercise Type objects and friends, we'll use a single TypeLoader to keep the test methods from being
        // bound too tightly to the fact that the Types are coming from a TypeLoader.
        //
        // This is a simple TypeLoader designed to do the minimal work to make test assembly work across various runtime scenarios.
        ...
        private static readonly Lazy<bool> s_useRuntimeTypesForTests = new Lazy<bool>(() =>
        {
            if (File.Exists(Path.Combine(Path.GetDirectoryName(typeof(TestUtils).Assembly.Location), "UseRuntimeTypes.txt")))
            {
                ...
                Console.WriteLine("! Running Reflection tests on runtime types. They should pass but the results say nothing about TypeLoader correctness.");
                Console.ResetColor();
                return true;
            }
            return false;
