...
#endif
{
    public partial class EventSource
    {
                ...
                for (int i = 0; i < m_traits.Length - 1; i += 2)
                {
                    if (m_traits[i].StartsWith("ETW_", StringComparison.Ordinal))
                    {
                        ...
                        if (!byte.TryParse(etwTrait, out traitNum))
                        {
                            ...
                            {
                                throw new ArgumentException(SR.Format(SR.UnknownEtwTrait, etwTrait), "traits");
                            }
                        }
        ...
        private static int AddValueToMetaData(List<byte> metaData, string value)
        {
            ...
            else if (firstChar == '#')
            {
                for (int i = 1; i < value.Length; i++)
                {
                    if (value[i] != ' ')        // Skip spaces between bytes.  
                    {
                        if (!(i + 1 < value.Length))
                        {
                            throw new ArgumentException(SR.EvenHexDigits, "traits");
                        }
            ...
            {
                throw new ArgumentException(SR.Format(SR.IllegalValue, value), "traits");
            }
        ...
        private static int HexDigit(char c)
        {
            ...
            throw new ArgumentException(SR.Format(SR.BadHexDigit, c), "traits");
        }
