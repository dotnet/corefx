...
namespace System.Composition.Hosting.Core
{
    class ExportDescriptorRegistryUpdate : DependencyAccessor
    {
        ...
        private static readonly CompositionDependency[] _NoDependenciesValue = new CompositionDependency[0];
        private static readonly Func<CompositionDependency[]> _NoDependencies = () => _NoDependenciesValue;
        ...
        protected override IEnumerable<ExportDescriptorPromise> GetPromises(CompositionContract contract)
        {
            ...
            if (_partDefinitions.TryGetValue(contract, out definitions))
                return definitions.Select(d => new ExportDescriptorPromise(contract, "Preexisting", false, _NoDependencies, _ => d)).ToArray();
