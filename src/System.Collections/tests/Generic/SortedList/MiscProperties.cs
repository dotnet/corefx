...
namespace SortedListMiscProp
{
    public class Driver<KeyType, ValueType>
    {
        public void TestReadOnly(KeyType[] keys, ValueType[] values)
        {
            ...
            Test.Eval(false == ((IDictionary<KeyType, ValueType>)_dic).IsReadOnly, String.Format("Err_345sdg! wrong value returned, {0}", ((IDictionary<KeyType, ValueType>)_dic).IsReadOnly));
            for (int i = 0; i < keys.Length; i++)
                _dic.Add(keys[i], values[i]);
            Test.Eval(false == ((IDictionary<KeyType, ValueType>)_dic).IsReadOnly, String.Format("Err_0347tsg! wrong value returned, {0}", ((IDictionary<KeyType, ValueType>)_dic).IsReadOnly));
            _dic.Clear();
            Test.Eval(false == ((IDictionary<KeyType, ValueType>)_dic).IsReadOnly, String.Format("Err-34sg! wrong value returned, {0}", ((IDictionary<KeyType, ValueType>)_dic).IsReadOnly));
        }
        ...
        public void NonGenericIDictionaryTestFixedSize(KeyType[] keys, ValueType[] values)
        {
            ...
            Test.Eval(false == _idic.IsFixedSize, String.Format("Err_3234897agf! wrong value returned, {0}", _idic.IsFixedSize));
            for (int i = 0; i < keys.Length; i++)
                _dic.Add(keys[i], values[i]);
            Test.Eval(false == _idic.IsFixedSize, String.Format("Err_33467tsgb! wrong value returned, {0}", _idic.IsFixedSize));
            _dic.Clear();
            Test.Eval(false == _idic.IsFixedSize, String.Format("Err_33467tsgb! wrong value returned, {0}", _idic.IsFixedSize));
        }
        ...
        public void NonGenericIDictionaryTestReadOnly(KeyType[] keys, ValueType[] values)
        {
            SortedList<KeyType, ValueType> _dic = new SortedList<KeyType, ValueType>();
            IDictionary _idic = _dic;
            Test.Eval(false == _idic.IsReadOnly, String.Format("Err_4345sdg! wrong value returned, {0}", _idic.IsReadOnly));
            for (int i = 0; i < keys.Length; i++)
                _dic.Add(keys[i], values[i]);
            Test.Eval(false == _idic.IsReadOnly, String.Format("Err_40347tsg! wrong value returned, {0}", _idic.IsReadOnly));
            _dic.Clear();
            Test.Eval(false == _idic.IsReadOnly, String.Format("Err-434sg! wrong value returned, {0}", _idic.IsReadOnly));
        }
    }
