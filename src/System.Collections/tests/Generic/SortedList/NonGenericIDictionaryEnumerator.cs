...
namespace SortedListNoGenIDicEnum
{
    public class Driver<K, V> where K : IComparableValue
    {
        public void GetEnumeratorBasic(K[] keys, V[] values)
        {
            ...
            for (int i = 0; i < keys.Length; i++)
            {
                Test.Eval(Enum.MoveNext(), "Err_19191haiede Expected MoveNext to return true");
                DictionaryEntry entry = Enum.Entry;

                Test.Eval(kls.Contains((K)entry.Key), "Err_21988932haiue Expected Contains with key " + entry.Key + " to return true");
                kls.Remove((K)Enum.Key);
                Test.Eval(vls.Contains((V)entry.Value), "Err_2919ahied Epected Contains with value " + entry.Value + " to return true");
            ...
            {
                Test.Eval(kls.Contains((K)((DictionaryEntry)entry).Key), "Err_789322ahie Expected Contains with key " + (K)((DictionaryEntry)entry).Key + " to return true");
                kls.Remove((K)((DictionaryEntry)entry).Key);
                Test.Eval(vls.Contains((V)((DictionaryEntry)entry).Value), "Err_668851ajied Epected Contains with value " + (V)((DictionaryEntry)entry).Value + " to return true");
            ...
            for (int i = 0; i < keys.Length; i++)
                vls.Add(values[i]);
            Test.Eval(Enum.MoveNext() == false, "Err_29829anie Expected MoveNext to return false");
        }
        public void GetEnumeratorValidations(K[] keys, V[] values, K nkey, V nvalue)
        {
            ...
            for (int i = 0; i < keys.Length; i++)
            {
                Test.Eval(Enum.MoveNext(), "Err_19273ajeid Expected MoveNext to return true");
                entry = Enum.Entry;
                //			Test.Eval(entry.Key.Equals(keys[i]) && ( ((null==(object)entry.Value)&&(null==(object)values[i])) || entry.Value.Equals(values[i])));
            }
            Test.Eval(Enum.MoveNext() == false, "Err_32218ahied Expected MoveNext to return false");

            try
            {
                entry = Enum.Entry;
                Test.Eval(false, "Err_8795188ahied Expected Entry to throw InvalidOperationException");
            }
            ...
            {
                entry = Enum.Key;
                Test.Eval(false, "Err_584848ajeoia Expected Key to throw InvalidOperationException");
            }
            catch (InvalidOperationException)
            {
            }
            ...
            {
                entry = Enum.Value;
                Test.Eval(false, "Err_681588aiepad Expected Value to throw InvalidOperationException");
            }
            catch (InvalidOperationException)
            {
            }
            ...
            if (keys.Length != 0)
            {
                ...
                Test.Eval(entry.Equals(expectedDictionaryEntry), "Err_845418aheid Expected DictionaryEntry={0} actual={1}", expectedDictionaryEntry, entry);


                entry = Enum.Key;
                Test.Eval(entry.Equals(keys[0]), "Err_56688ajioed Expected Key={0} actual={1}", keys[0], entry);

                entry = Enum.Value;
                Test.Eval(entry.Equals(values[0]), "Err_5888ajoed Expected Value={0} actual={1}", values[0], entry);
            }
            else
            {
                try
                {
                    entry = Enum.Entry;
                    Test.Eval(false, "Err_845418aheid Expected Entry to throw InvalidOperationException");
                }
                catch (InvalidOperationException)
                {
                }
                ...
                {
                    entry = Enum.Key;
                    Test.Eval(false, "Err_56688ajioed Expected Key to throw InvalidOperationException");
                }
                catch (InvalidOperationException)
                {
                }
                ...
                {
                    entry = Enum.Value;
                    Test.Eval(false, "Err_5888ajoed Expected Value to throw InvalidOperationException");
                }
                catch (InvalidOperationException)
                {
                }
