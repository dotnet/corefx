...
namespace List_List_MiscMethods
{
    public class Driver<T>
    {
        ...
        public void InsertRangeICollection(T[] itemsX, T[] itemsY, int index, int repeat, Func<T[], IEnumerable<T>> constructIEnumerable)
        {
            ...
            //[] Verify that List does not pass the internal array to CopyTo
            list = new List<T>();
            for (int i = 0; i < 64; ++i)
            {
                list.Add(default(T));
            ...
            MyCollection<T> myCollection = new MyCollection<T>(tempList);
            list.InsertRange(4, myCollection);

            Assert.Equal(0, myCollection.StartIndex); //"Err_0518ahed: Expected "
            Assert.Equal(4, myCollection.Array.Length); //"Err_045ahed: Expected "
        }
    ...
    public class List_MiscMethods
    {
        ...
        public static void ToArrayTests()
        {
            ...
            RefDriver.BasicToArray(intArr1);
            RefDriver.BasicToArray(new RefX1<int>[] { });
            RefDriver.EnsureNotUnderlyingToArray(intArr1, null);
            RefDriver.EnsureNotUnderlyingToArray(intArr1, new RefX1<int>(100));
            ...
            ValDriver.BasicToArray(vintArr1);
            ValDriver.BasicToArray(new ValX1<int>[] { });
            ValDriver.EnsureNotUnderlyingToArray(vintArr1, new ValX1<int>(100));
        }
    }
    ...
    public class MyCollection<T> : ICollection<T>
    {
        ...
        public MyCollection(ICollection<T> collection)
        {
