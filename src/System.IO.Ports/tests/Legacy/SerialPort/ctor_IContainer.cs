...
public class ctor
{
    ...
    public bool Verify()
    {
        ...
        SerialPort com = new SerialPort(container);
        bool retValue = true;

        if (container.Components.Count != 1)
        {
            Console.WriteLine("Expected container.Components.Count=1 actual={0}", container.Components.Count);
            retValue = false;
        }
        else if (container.Components[0] != com)
        {
            Console.WriteLine("Expected container.Components[0]=SerialPort actual={0}", container.Components[0].GetType());
            retValue = false;
        }
