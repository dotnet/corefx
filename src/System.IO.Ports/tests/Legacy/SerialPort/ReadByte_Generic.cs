...
public class ReadByte_Generic : PortsTest
{
    ...
    public bool NoParityReplaceByte()
    {
        ...
        if (!VerifyParityReplaceByte((int)'\0', rndGen.Next(0, numRndByte - 1), new System.Text.UTF32Encoding()))
        {
    ...
    public bool ParityErrorOnLastByte()
    {
        ...
        for (int i = 0; i < expectedBytes.Length; i++)
        {
            ...
            {
                System.Debug.WriteLine("ERROR!!!: Expected to read {0}  actual read  {1}", (int)expectedBytes[i], (int)actualBytes[i]);
                retValue = false;
            }
        }
        ...
        if (!retValue)
            Debug.WriteLine("Err_010!!! Verifying default ParityReplace byte with a parity error on the last byte failed");
    ...
    private bool VerifyTimeout(SerialPort com)
    {
        ...
        try
        {
            com.ReadByte(); //Warm up read method
    ...
    private bool VerifyRead(SerialPort com1, SerialPort com2, byte[] bytesToWrite, byte[] expectedBytes, System.Text.Encoding encoding)
    {
        ...
        while (true)
        {
            ...
            {
                System.Debug.WriteLine("ERROR!!!: Expected BytesToRead={0} actual={1}", bytesToWrite.Length - rcvBufferSize, com1.BytesToRead);
                retValue = false;
            }
            ...
            if (readInt != expectedBytes[i])
            {
                ...
                System.Debug.WriteLine("ERROR!!!: Expected to read {0}  actual read byte {1}", expectedBytes[i], (byte)readInt);
                retValue = false;
            }
        ...
        {
            Debug.WriteLine("ERROR!!! Expected to read {0} char actually read {1} chars", bytesToWrite.Length, rcvBufferSize);
            retValue = false;
        }
