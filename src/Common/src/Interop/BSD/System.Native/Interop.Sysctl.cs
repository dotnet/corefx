...
internal static partial class Interop
{
    internal static partial class Sys
    {
        ...
        private extern static unsafe int SysctlByName(string name, void* value, size_t* len);
        ...
        public static unsafe int Sysctl(string name, ref byte* value, ref int len)
        {
            ...
            {
                // do one try to see how much data we need
                ret = SysctlByName(name, pBuffer, &bytesLength);
                if (ret != 0)
                {
                    throw new InvalidOperationException(string.Format("sysctl returned {0}", ret));
                }
        ...
        // makes sysctl() call by name and returns string.
        public static unsafe int Sysctl(string name, out string value)
        {
            IntPtr bytesLength = (IntPtr)0;
            byte* pBuffer = null;
            value = null;

            // do one try to see how much data we need
            int ret = SysctlByName(name,  pBuffer, &bytesLength);
            if (ret != 0)
            {
                throw new InvalidOperationException(string.Format("sysctl returned {0}", ret));
            }
            try
            {
                ...
                value = Marshal.PtrToStringAnsi((IntPtr)pBuffer);
            }
            finally
            {
                Marshal.FreeHGlobal((IntPtr)pBuffer);
