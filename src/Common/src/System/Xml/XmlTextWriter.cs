...
namespace System.Xml
{
    ...
    internal class XmlTextWriter : XmlWriter
    {
        ...
        private static readonly State[] _stateTableDefault = {
        ...
        private static readonly State[] _stateTableDocument = {
            //                          State.Start      State.Prolog     State.PostDTD    State.Element    State.Attribute  State.Content   State.AttrOnly   State.Epilog
            //
        ...
        internal XmlTextWriter()
        {
            ...
            _stateTable = _stateTableDefault;
        ...
        public XmlTextWriter(Stream w, Encoding encoding) : this()
        {
            ...
            _xmlEncoder.QuoteChar = this._quoteChar;
        }
        ...
        public bool Namespaces
        {
            ...
            set
            {
                if (this._currentState != State.Start)
        ...
        public Formatting Formatting
        {
            ...
            set { this._formatting = value; this._indented = value == Formatting.Indented; }
        }
        ...
        public char QuoteChar
        {
            ...
            set
            {
                ...
                this._xmlEncoder.QuoteChar = value;
            }
        }
        ...
        public override void WriteEndDocument()
        {
            ...
            {
                AutoCompleteAll();
                if (this._currentState != State.Epilog)
                {
                ...
                this._stateTable = _stateTableDefault;
                this._currentState = State.Start;
                this._lastToken = Token.Empty;
            }
            catch
            {
        ...
        public override void WriteStartAttribute(string prefix, string localName, string ns)
        {
            try
            {
                ...
                this._specialAttr = SpecialAttr.None;
                if (this._namespaces)
                {
                    if (prefix != null && prefix.Length == 0)
                    {
                    ...
                    if (prefix == "xml")
                    {
                        if (localName == "lang")
                        {
                            this._specialAttr = SpecialAttr.XmlLang;
                        }
                        else if (localName == "space")
                        {
                            this._specialAttr = SpecialAttr.XmlSpace;
                        }
                    }
                    else if (prefix == "xmlns")
                    {
                        ...
                        this._specialAttr = SpecialAttr.XmlNs;
                    }
                ...
                _xmlEncoder.StartAttribute(this._specialAttr != SpecialAttr.None);

                _textWriter.Write(localName);
                _textWriter.Write('=');
                if (this._curQuoteChar != this._quoteChar)
                {
                    this._curQuoteChar = this._quoteChar;
                    _xmlEncoder.QuoteChar = this._quoteChar;
                }
                _textWriter.Write(this._curQuoteChar);
            }
            catch
            {
                _currentState = State.Error;
        ...
        public override void WriteProcessingInstruction(string name, string text)
        {
            try
            {
                ...
                if (0 == String.Compare(name, "xml", StringComparison.OrdinalIgnoreCase) && this._stateTable == _stateTableDocument)
                {
        ...
        public override void WriteBase64(byte[] buffer, int index, int count)
        {
            try
            {
                ...
                // it for us.
                if (null == this._base64Encoder)
                {
                    this._base64Encoder = new XmlTextWriterBase64Encoder(_xmlEncoder);
                }
                // Encode will call WriteRaw to write out the encoded characters
                this._base64Encoder.Encode(buffer, index, count);
            }
            catch
            {
                _currentState = State.Error;
        ...
        {
            if (disposing && this._currentState != State.Closed)
            {
                try
                {
                    AutoCompleteAll();
            ...
            try
            {
                ...
                if (this._namespaces)
                {
                    if (ns != null && ns.Length != 0 && ns != _stack[_top].defaultNs)
                    {
                        ...
                        if (prefix == null)
                        {
                            if (this._currentState != State.Attribute)
                            {
        ...
        void StartDocument(int standalone)
        {
            try
            {
                ...
                this._stateTable = _stateTableDocument;
                this._currentState = State.Prolog;
                ...
                if (this._encoding != null)
                {
                    ...
                    bufBld.Append(this._encoding.WebName);
                    bufBld.Append(_quoteChar);
                }
                if (standalone >= 0)
                {
        ...
        void AutoComplete(Token token)
        {
            ...
            else if (this._currentState == State.Error)
            {
                throw new InvalidOperationException(SR.Format(SR.Xml_WrongToken, _tokenName[(int)token], _stateName[(int)State.Error]));
            }

            State newState = this._stateTable[(int)token * 8 + (int)this._currentState];
            if (newState == State.Error)
            {
                throw new InvalidOperationException(SR.Format(SR.Xml_WrongToken, _tokenName[(int)token], _stateName[(int)this._currentState]));
            }

            switch (token)
            {
                case Token.Doctype:
                    if (this._indented && this._currentState != State.Start)
                    {
                        Indent(false);
                    }
                    break;
                ...
                case Token.CData:
                    ...
                    else if (this._currentState == State.Element)
                    {
                        WriteEndStartTag(false);
                    }
                    ...
                    else if (this._indented && this._currentState != State.Start)
                    {
                        Indent(false);
                    }
                    break;
                ...
                case Token.LongEndElement:
                    ...
                    if (this._currentState == State.Content)
                    {
                    ...
                    if (_stateTableDocument == this._stateTable && _top == 1)
                    {
                ...
                case Token.Base64:
                    ...
                    if (token != Token.Base64 && this._flush)
                    {
                        FlushEncoders();
                    }
                    if (this._currentState == State.Element && this._lastToken != Token.Content)
                    {
                        WriteEndStartTag(false);
                    }
        ...
        void InternalWriteEndElement(bool longFormat)
        {
            try
            {
                ...
                if (this._lastToken == Token.LongEndElement)
                {
                    ...
                    _textWriter.Write('<');
                    _textWriter.Write('/');
                    if (this._namespaces && _stack[_top].prefix != null)
                    {
        ...
        void WriteEndStartTag(bool empty)
        {
            ...
            for (int i = _nsTop; i > _stack[_top].prevNsTop; i--)
            {
                if (!_nsStack[i].declared)
                {
                    ...
                    _textWriter.Write('=');
                    _textWriter.Write(this._quoteChar);
        ...
        {
            if (this._specialAttr != SpecialAttr.None)
            {
        ...
        void Indent(bool beforeEndElement)
        {
            ...
            else if (!_stack[_top].mixed)
            {
                ...
                for (i *= this._indentation; i > 0; i--)
                {
                    _textWriter.Write(this._indentChar);
                }
            }
        }
        ...
        void HandleSpecialAttribute()
        {
            ...
            switch (this._specialAttr)
            {
                ...
                case SpecialAttr.XmlNs:
                    VerifyPrefixXml(this._prefixForXmlNs, value);
                    PushNamespace(this._prefixForXmlNs, value, true);
                    break;
            }
        }
