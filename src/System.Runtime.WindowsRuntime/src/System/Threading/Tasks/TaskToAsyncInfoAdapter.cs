...
#pragma warning disable 0420
        ...
        private UInt32 m_id = AsyncInfoIdGenerator.InvalidId;
        ...
        private volatile Int32 m_state = STATE_NOT_INITIALIZED;
        ...
        internal TaskToAsyncInfoAdapter(Delegate taskProvider)
        {
            ...
            this.m_startingContext = GetStartingContext();
            ...
            this.m_dataContainer = task;
            this.m_state = (STATEFLAG_COMPLETION_HNDL_NOT_YET_INVOKED | STATE_STARTED);
                                        ...
                                        CancellationTokenSource underlyingCancelTokenSource, Progress<TProgressInfo> underlyingProgressDispatcher)
        {
            ...
            this.m_dataContainer = underlyingTask;
            ...
            this.m_cancelTokenSource = underlyingCancelTokenSource;
        ...
        internal TaskToAsyncInfoAdapter(TResult synchronousResult)
        {
            ...
            this.m_startingContext = null;

            // Set the synchronous result:
            this.m_dataContainer = synchronousResult;
            ...
            this.m_state = (STATEFLAG_COMPLETED_SYNCHRONOUSLY
        ...
        internal bool DangerousSetCompleted(TResult synchronousResult)
        {
            ...
            m_dataContainer = synchronousResult;
            m_error = null;
        ...
        internal bool CompletedSynchronously {[Pure] get { return (0 != (m_state & STATEFLAG_COMPLETED_SYNCHRONOUSLY)); } }

        private bool IsInStartedState {[Pure] get { return (0 != (m_state & STATE_STARTED)); } }

        private bool IsInRunToCompletionState {[Pure] get { return (0 != (m_state & STATE_RUN_TO_COMPLETION)); } }

        private bool IsInErrorState {[Pure] get { return (0 != (m_state & STATE_ERROR)); } }

        private bool IsInClosedState {[Pure] get { return (0 != (m_state & STATE_CLOSED)); } }
        ...
        private bool IsInRunningState
        {
            ...
            get
            {
                return (0 != (m_state & (STATE_STARTED
        ...
        private bool IsInTerminalState
        {
            ...
            get
            {
                return (0 != (m_state & (STATE_RUN_TO_COMPLETION
                                       | STATE_CANCELLATION_COMPLETED
                                       | STATE_ERROR)));
            }
        }
        ...
        SynchronizationContext GetStartingContext()
        {
        ...
        {
            get { return m_cancelTokenSource; }
        }
        ...
        {
            Contract.Assert(false, "This (sub-)type of IAsyncInfo does not support completion notifications "
        ...
        {
            Contract.Assert(false, "This (sub-)type of IAsyncInfo does not support progress notifications "
                                 + " (" + this.GetType().ToString() + ")");
        }
        ...
        private void OnCompletedInvoker(AsyncStatus status)
        {
            ...
            // Get the handler:
            TCompletedHandler handler = Volatile.Read(ref m_completedHandler);
            ...
            if (handler == null)
            {
                ...
                handler = Volatile.Read(ref m_completedHandler);
        ...
        private void OnProgressInvokerCrossContext(TProgressHandler handler, TProgressInfo progressInfo)
        {
            ...
            Contract.Requires(m_startingContext != null);

            m_startingContext.Post((tupleObject) =>
            {
        ...
        void IProgress<TProgressInfo>.Report(TProgressInfo value)
        {
            ...
            TProgressHandler handler = Volatile.Read(ref m_progressHandler);
            if (handler == null)
                return;
        ...
        {
            Contract.Assert(CheckUniqueAsyncState(newAsyncState & STATEMASK_SELECT_ANY_ASYNC_STATE));
            Contract.Assert(CheckUniqueAsyncState(m_state & STATEMASK_SELECT_ANY_ASYNC_STATE));
            ...
            Contract.Assert(CheckUniqueAsyncState(resultState & STATEMASK_SELECT_ANY_ASYNC_STATE));
        ...
        private Int32 SetState(Int32 newStateSetMask, Int32 newStateIgnoreMask, Int32 conditionBitMask, bool useCondition, out bool conditionFailed)
        {
            ...
            Int32 prevState = Interlocked.CompareExchange(ref m_state, newState, origState);
            ...
            while (true)
            {
                ...
                prevState = Interlocked.CompareExchange(ref m_state, newState, origState);
            }
        }
        ...
        {
            Contract.Assert(IsInRunningState);
            Contract.Assert(!CompletedSynchronously);

            Task task = m_dataContainer as Task;
            ...
            switch (task.Status)
            {
                ...
                default:
                    Contract.Assert(false, "Unexpected task.Status: It should be terminal if TaskCompleted() is called.");
                    break;
            }
            ...
            Contract.Assert((newState & STATEMASK_SELECT_ANY_ASYNC_STATE) == terminalAsyncState);
            Contract.Assert((m_state & STATEMASK_SELECT_ANY_ASYNC_STATE) == terminalAsyncState || IsInClosedState,
        ...
        private void TaskCompleted()
        {
            ...
            Contract.Assert(IsInTerminalState);
            ...
            {
                ExceptionDispatchHelper.ThrowAsync(ex, m_startingContext);
            }
        }
        ...
        private AsyncStatus GetStatus(Int32 state)
        {
            ...
            Contract.Assert(CheckUniqueAsyncState(asyncState));

            switch (asyncState)
            {
                case STATE_NOT_INITIALIZED:
                    Contract.Assert(false, "STATE_NOT_INITIALIZED should only occur when this object was not"
                ...
                case STATE_CLOSED:
                    Contract.Assert(false, "This method should never be called is this IAsyncInfo is CLOSED");
                    return AsyncStatus.Error;
            }

            Contract.Assert(false, "The switch above is missing a case");
            return AsyncStatus.Error;
        }
        ...
        internal TResult GetResultsInternal()
        {
            ...
            if (CompletedSynchronously)
                return (TResult)m_dataContainer;

            // The operation is asynchronous:
            Task<TResult> task = m_dataContainer as Task<TResult>;
            ...
            Contract.Assert(IsInRunToCompletionState);
        ...
        private Task InvokeTaskProvider(Delegate taskProvider)
        {
            ...
            if (funcCTokTask != null)
            {
                ...
                return funcCTokTask(m_cancelTokenSource.Token);
            }
            ...
            {
                m_cancelTokenSource = new CancellationTokenSource();
                return funcCTokIPrgrTask(m_cancelTokenSource.Token, this);
            }
        ...
        private void TransitionToClosed()
        {
            ...
            m_error = null;
            m_completedHandler = null;
            m_progressHandler = null;
            m_startingContext = null;
        }
        ...
        public virtual TCompletedHandler Completed
        {
            ...
            set
            {
                ...
                TCompletedHandler handlerBefore = Interlocked.CompareExchange(ref m_completedHandler, value, null);
                ...
                if (0 == (m_state & STATEFLAG_MUST_RUN_COMPLETION_HNDL_WHEN_SET))
                    return;
                ...
                if (IsInClosedState)
                {
                    Interlocked.Exchange(ref m_completedHandler, null);
                    return;
                }
        ...
        public virtual TProgressHandler Progress
        {
            ...
            set
            {
                ...
                Interlocked.Exchange(ref m_progressHandler, value);

                // We we transitioned into CLOSED after the above check, we will need to null out m_progressHandler:
                if (IsInClosedState)
                    Interlocked.Exchange(ref m_progressHandler, null);
            }
        }
        ...
        public virtual void Close()
        {
            ...
            if (!IsInTerminalState)
            {
                ...
                if (0 != (m_state & STATEMASK_SELECT_ANY_ASYNC_STATE))
                {
        ...
        public virtual Exception ErrorCode
        {
            get
            {
                ...
                Exception error = Volatile.Read(ref m_error);
                ...
                Exception setError = Interlocked.CompareExchange(ref m_error, error, null);
        ...
        public virtual UInt32 Id
        {
            get
            {
                ...
                if (m_id != AsyncInfoIdGenerator.InvalidId)
                    return m_id;

                return AsyncInfoIdGenerator.EnsureInitializedThreadsafe(ref m_id);
            }
        }
