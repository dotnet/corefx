...
namespace System
{
    public static partial class PlatformDetection
    {
        ...
        private static bool IsDistroAndVersionOrHigher(Predicate<string> distroPredicate, int major = -1, int minor = -1, int build = -1, int revision = -1)
        {
            ...
            {
                DistroInfo v = GetDistroInfo();
                if (distroPredicate(v.Id) && VersionEquivalentWithOrHigher(major, minor, build, revision, v.VersionId))
                {
                    return true;
                }
            }
        ...
        private static bool VersionEquivalentWithOrHigher(int major, int minor, int build, int revision, Version actualVersionId)
        {
            return 
                VersionEquivalentWith(major, minor, build, revision, actualVersionId) ||
