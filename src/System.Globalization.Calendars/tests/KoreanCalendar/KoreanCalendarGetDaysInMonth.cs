...
namespace System.Globalization.CalendarsTests
{
    ...
    public class KoreanCalendarGetDaysInMonth
    {
        ...
        public void GetDaysInMonth_Invalid()
        {
            ...
            Assert.Throws<ArgumentOutOfRangeException>(() => new KoreanCalendar().GetDaysInMonth(1, 10, 1));
            Assert.Throws<ArgumentOutOfRangeException>(() => new KoreanCalendar().GetDaysInMonth(-1, 10, 1));
            Assert.Throws<ArgumentOutOfRangeException>(() => new KoreanCalendar().GetDaysInMonth(2333, 10, 1));
            Assert.Throws<ArgumentOutOfRangeException>(() => new KoreanCalendar().GetDaysInMonth(12333, 10, 1));

            // Month is outside supported range
            Assert.Throws<ArgumentOutOfRangeException>(() => new KoreanCalendar().GetDaysInMonth(2334, -1, 1));
            Assert.Throws<ArgumentOutOfRangeException>(() => new KoreanCalendar().GetDaysInMonth(2334, 0, 1));
            Assert.Throws<ArgumentOutOfRangeException>(() => new KoreanCalendar().GetDaysInMonth(2334, 13, 1));

            // Era is outside supported range
            Assert.Throws<ArgumentOutOfRangeException>(() => new KoreanCalendar().GetDaysInMonth(2334, 10, -1));
            Assert.Throws<ArgumentOutOfRangeException>(() => new KoreanCalendar().GetDaysInMonth(2334, 10, 2));
        }
    }
}
