...
{
    // NOTE: For now these stat functions are implemented as P/Invoke wrappers for the native OS
    // functionality, but the stat data structure has non-trivial variations from platform to
    // platform.  In the future, we might want to introduce a very thin native PAL layer
