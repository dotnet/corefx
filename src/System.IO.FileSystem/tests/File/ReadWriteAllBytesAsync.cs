...
namespace System.IO.Tests
{
    public class File_ReadWriteAllBytesAsync : FileSystemTest
    {
        ...
        public async Task NullParametersAsync()
        {
            ...
            await Assert.ThrowsAsync<ArgumentNullException>(async () => await File.WriteAllBytesAsync(null, new byte[0]));
            await Assert.ThrowsAsync<ArgumentNullException>(async () => await File.WriteAllBytesAsync(path, null));
            await Assert.ThrowsAsync<ArgumentNullException>(async () => await File.ReadAllBytesAsync(null));
        }
        ...
        {
            string path = GetTestFilePath();
            await Assert.ThrowsAsync<ArgumentException>(async () => await File.WriteAllBytesAsync(string.Empty, new byte[0]));
            await Assert.ThrowsAsync<ArgumentException>(async () => await File.ReadAllBytesAsync(string.Empty));
        }
