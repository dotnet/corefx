...
namespace System.IO.Tests
{
    public class File_ReadWriteAllTextAsync : FileSystemTest
    {
        ...
        {
            string path = GetTestFilePath();
            await Assert.ThrowsAsync<ArgumentException>(async () => await WriteAsync(string.Empty, "Text"));
            await Assert.ThrowsAsync<ArgumentException>(async () => await ReadAsync(""));
        }
    ...
    public class File_ReadWriteAllText_EncodedAsync : File_ReadWriteAllTextAsync
    {
        ...
        {
            string path = GetTestFilePath();
            await Assert.ThrowsAsync<ArgumentNullException>(async () => await File.WriteAllTextAsync(path, "Text", null));
            await Assert.ThrowsAsync<ArgumentNullException>(async () => await File.ReadAllTextAsync(path, null));
        }
