...
namespace System.Net.Http
{
    ...
    internal struct ArrayBuffer
    {
        ...
        public Span<byte> ReadBytes => new Span<byte>(_bytes, _readPosition, _writePosition - _readPosition);
        public Span<byte> WriteBytes => new Span<byte>(_bytes, _writePosition, _bytes.Length - _writePosition);
        ...
        {
            Debug.Assert(byteCount <= ReadBytes.Length);
        ...
        {
            Debug.Assert(byteCount <= WriteBytes.Length);
        ...
        {
            if (byteCount <= WriteBytes.Length)
            {
                return;
            }

            int totalFree = _readPosition + WriteBytes.Length;
            if (byteCount <= totalFree)
            {
                // We can free up enough space by just shifting the bytes down, so do so.
                Buffer.BlockCopy(_bytes, _readPosition, _bytes, 0, ReadBytes.Length);
                _writePosition = ReadBytes.Length;
                _readPosition = 0;
                Debug.Assert(byteCount <= WriteBytes.Length);
                return;
            }
            ...
            int desiredSize = ReadBytes.Length + byteCount;
            ...
            {
                Buffer.BlockCopy(_bytes, _readPosition, newBytes, 0, ReadBytes.Length);
            }
