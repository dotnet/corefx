        ...
        private static string GetResourceString(ref Utf8JsonReader json, ExceptionResource resource, byte nextByte, string characters)
        {
            ...
            switch (resource)
            {
                case ExceptionResource.ArrayDepthTooLarge:
                    message = SR.Format(SR.ArrayDepthTooLarge, json.ReaderDepth + 1, json.CurrentState.Options.MaxDepth);
                    break;
                ...
                case ExceptionResource.ObjectDepthTooLarge:
                    message = SR.Format(SR.ObjectDepthTooLarge, json.ReaderDepth + 1, json.CurrentState.Options.MaxDepth);
                    break;
                ...
                case ExceptionResource.ZeroDepthAtEnd:
                    message = SR.Format(SR.ZeroDepthAtEnd, json.ReaderDepth);
                    break;
