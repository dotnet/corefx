...
namespace System.Text.Json.Serialization.Tests
{
    public static partial class OptionsTests
    {
        ...
        {
            Assert.Throws<JsonSerializationException>(() => JsonSerializer.Parse<int[]>("[1,]"));
        ...
        {
            Assert.Throws<JsonSerializationException>(() => JsonSerializer.Parse<object>("/* commment */"));

            var options = new JsonSerializerOptions();

            Assert.Throws<JsonSerializationException>(() => JsonSerializer.Parse<object>("/* commment */", options));
        ...
        public static void MaxDepthRead()
        {
            ...
            Assert.Throws<JsonSerializationException>(() => JsonSerializer.Parse<BasicCompany>(BasicCompany.s_data, options));
        }
    }
}
