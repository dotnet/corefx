...
namespace System.Text.Json.Tests
{
    public static class JsonDocumentTests
    {
        ...
        public static void ParseJson_SeekableStream_BadBOM(string json)
        {
            ...
            Assert.Throws<JsonException>(() => JsonDocument.Parse(new MemoryStream(data)));
        }
        ...
        {
            byte[] data = Encoding.UTF8.GetBytes(json);
            return Assert.ThrowsAsync<JsonException>(() => JsonDocument.ParseAsync(new MemoryStream(data)));
        }
        ...
        {
            Assert.Throws<JsonException>(() => JsonDocument.Parse(json));

            byte[] utf8 = Encoding.UTF8.GetBytes(json);
            Assert.Throws<JsonException>(() => JsonDocument.Parse(utf8));

            ReadOnlySequence<byte> singleSeq = new ReadOnlySequence<byte>(utf8);
            Assert.Throws<JsonException>(() => JsonDocument.Parse(singleSeq));

            ReadOnlySequence<byte> multiSegment = JsonTestHelper.SegmentInto(utf8, 6);
            Assert.Throws<JsonException>(() => JsonDocument.Parse(multiSegment));

            Stream stream = new MemoryStream(utf8);
            Assert.Throws<JsonException>(() => JsonDocument.Parse(stream));

            stream.Seek(0, SeekOrigin.Begin);
            return Assert.ThrowsAsync<JsonException>(() => JsonDocument.ParseAsync(stream));
        }
        ...
        public static void CheckParseDepth()
        {
            ...
            Assert.Throws<JsonException>(() => JsonDocument.Parse(badJson));
        }
        ...
        public static void HonorReaderOptionsMaxDepth()
        {
            ...
            Assert.Throws<JsonException>(() => JsonDocument.Parse(okayJson, new JsonReaderOptions { MaxDepth = 32 }));
            Assert.Throws<JsonException>(() => JsonDocument.Parse(okayJson));
            Assert.Throws<JsonException>(() => JsonDocument.Parse(okayJson, new JsonReaderOptions { MaxDepth = 0 }));
            Assert.Throws<JsonException>(() => JsonDocument.Parse(okayJson, new JsonReaderOptions { MaxDepth = 64 }));
        }
