    ...
    internal class RuntimeBinder
    {
            ...
            out DynamicMetaObject deferredBinding)
        {
            ...
            if (payload is CSharpInvokeMemberBinder)
            {
                ...
                if (swt != null && swt.Sym.getKind() != SymbolKind.SK_MethodSymbol)
                {
            ...
            SymbolKind kind)
        {
            ...
            switch (kind)
            {
                ...
                case SymbolKind.SK_IndexerSymbol:
            ...
            BindingFlag flags)
        {
            ...
            EXPRMEMGRP pMemGroup = CreateMemberGroupEXPR(property.name.Text, null, callingObject, SymbolKind.SK_PropertySymbol);
        ...
        private EXPR CreateIndexer(SymWithType swt, EXPR callingObject, EXPR arguments, BindingFlag bindFlags)
        {
            ...
            EXPRMEMGRP memgroup = CreateMemberGroupEXPR(index.name.Text, null, callingObject, SymbolKind.SK_PropertySymbol);
            ...
            Dictionary<int, LocalVariableSymbol> dictionary)
        {
            ...
            if (swt.Sym.getKind() != SymbolKind.SK_MethodSymbol)
            {
            ...
            if ((payload.Flags & CSharpCallFlags.EventHookup) != 0)
            {
                ...
                if (swtEvent.Sym.getKind() == SymbolKind.SK_FieldSymbol)
                {
                    eventCType = swtEvent.Field().GetType();
                }
                else if (swtEvent.Sym.getKind() == SymbolKind.SK_EventSymbol)
                {
                ...
                if (swtEvent.Sym.getKind() == SymbolKind.SK_EventSymbol && swtEvent.Event().IsWindowsRuntimeEvent)
                {
        ...
        private EXPR BindWinRTEventAccessor(EventWithType ewt, EXPR callingObject, ArgumentObject[] arguments, Dictionary<int, LocalVariableSymbol> dictionary, bool isAddAccessor)
        {
            ...
            EXPRMEMGRP addEventGrp = CreateMemberGroupEXPR(methodName, new List<Type> { evtType }, marshalClass, SymbolKind.SK_MethodSymbol);
            ...
            Dictionary<int, LocalVariableSymbol> dictionary)
        {
            ...
            if (swt != null && swt.Sym.getKind() == SymbolKind.SK_EventSymbol)
            {
                result = true;
            }
            ...
            if (swt != null && swt.Sym.getKind() == SymbolKind.SK_FieldSymbol && swt.Sym.AsFieldSymbol().isEvent)
            {
                result = true;
            }
