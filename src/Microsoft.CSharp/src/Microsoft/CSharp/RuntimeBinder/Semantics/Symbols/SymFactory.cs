...
namespace Microsoft.CSharp.RuntimeBinder.Semantics
{
    internal class SymFactory : SymFactoryBase
    {
        ...
        {
            NamespaceSymbol sym = newBasicSym(SymbolKind.SK_NamespaceSymbol, name, parent).AsNamespaceSymbol();
        ...
        public AssemblyQualifiedNamespaceSymbol CreateNamespaceAid(Name name, ParentSymbol parent, KAID assemblyID)
        {
            ...
            AssemblyQualifiedNamespaceSymbol sym = newBasicSym(SymbolKind.SK_AssemblyQualifiedNamespaceSymbol, name, parent).AsAssemblyQualifiedNamespaceSymbol();
        ...
        public AggregateSymbol CreateAggregate(Name name, NamespaceOrAggregateSymbol parent, InputFile infile, TypeManager typeManager)
        {
            ...
            {
                sym = newBasicSym(SymbolKind.SK_AggregateSymbol, name, parent).AsAggregateSymbol();
                sym.name = name;
        ...
        public AggregateDeclaration CreateAggregateDecl(AggregateSymbol agg, Declaration declOuter)
        {
            ...
            AggregateDeclaration sym = newBasicSym(SymbolKind.SK_AggregateDeclaration, agg.name, null).AsAggregateDeclaration();
        ...
        public AggregateSymbol CreateUnresolvedAggregate(Name name, ParentSymbol parent, TypeManager typeManager)
        {
            ...
            Symbol sym = newBasicSym(SymbolKind.SK_UnresolvedAggregateSymbol, name, parent);
            AggregateSymbol AggregateSymbol = null;

            // Unresolved Aggs need extra storage, but are still considered Aggs.

            sym.setKind(SymbolKind.SK_AggregateSymbol);
        ...
        public FieldSymbol CreateMemberVar(Name name, ParentSymbol parent, AggregateDeclaration declaration, int iIteratorLocal)
        {
            ...
            FieldSymbol sym = newBasicSym(SymbolKind.SK_FieldSymbol, name, parent).AsFieldSymbol();
        ...
        {
            LocalVariableSymbol sym = newBasicSym(SymbolKind.SK_LocalVariableSymbol, name, parent).AsLocalVariableSymbol();
            sym.SetType(type);
        ...
        {
            MethodSymbol sym = newBasicSym(SymbolKind.SK_MethodSymbol, name, parent).AsMethodSymbol();
            sym.declaration = declaration;
        ...
        {
            PropertySymbol sym = newBasicSym(SymbolKind.SK_PropertySymbol, name, parent).AsPropertySymbol();
            sym.declaration = declaration;
            Debug.Assert(sym != null);
            return (sym);
        }
        ...
        {
            EventSymbol sym = newBasicSym(SymbolKind.SK_EventSymbol, name, parent).AsEventSymbol();
            sym.declaration = declaration;
        ...
        {
            TypeParameterSymbol pResult = newBasicSym(SymbolKind.SK_TypeParameterSymbol, pName, pParent).AsTypeParameterSymbol();
