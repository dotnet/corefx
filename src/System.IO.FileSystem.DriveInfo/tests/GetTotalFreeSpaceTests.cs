...
{
    public class Get_TotalFreeSpace
    {
        ...
        public void Test01()
        {
            ...
            for (int i = 0; i < drives.Length; i++)
            {
                ...
                win32Values.Add(drives[i].Name, fbTotal);
            }
            ...
            for (int i = 0; i < drives.Length; i++)
            {
                ...
                if ((long)win32Values[drives[i].Name] != -1)
                {
                    ...
                    if (writable)
                    {
                        ...
                        if (size == (long)win32Values[drives[i].Name])
                        {
                            Assert.False(true, string.Format("Error, Wrong result returned. Expected: {1}, Returned: {2}", i, (long)win32Values[drives[i].Name], size));
                        }
            ...
            foreach (String letter in nonDriveLetters)
            {
                ...
                Assert.Throws<DriveNotFoundException>(() => { size = drive.TotalFreeSpace; });
            }
        }
    }
}
