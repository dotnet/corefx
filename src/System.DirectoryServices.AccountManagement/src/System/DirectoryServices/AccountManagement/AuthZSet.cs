...
namespace System.DirectoryServices.AccountManagement
{
    class AuthZSet : ResultSet
    {
            ...
            try
            {
                ...
                if (f)
                {
                    ...
                    f = UnsafeNativeMethods.AuthzInitializeContextFromSid(
                                                0,                  // default flags
                                                this.psUserSid.DangerousGetHandle(),
                                                pResManager,
                                                IntPtr.Zero,
                                                luid,
                                                IntPtr.Zero,
                    ...
                    if (f)
                    {
                        ...
                        if (!f && (bufferSize > 0) && (Marshal.GetLastWin32Error() == 122) /*ERROR_INSUFFICIENT_BUFFER*/)
                        {
                            ...
                            if (f)
                            {
                                ...
                                this.groupSidList = new SidList(groups);
                            }
                            else
                            {
                ...
                this.psBuffer = new SafeMemoryPtr(pBuffer);
                pBuffer = IntPtr.Zero;
            }
            catch (Exception e)
            {
    	...
    	override internal object CurrentAsPrincipal
    	{
            ...
            get
            {
                Debug.Assert(this.currentGroup >= 0 && this.currentGroup < this.groupSidList.Length);
                ...
                GlobalDebug.WriteLineIf(
                                ...
                                this.currentGroup,
                                this.groupSidList.Length);


                // Convert native SID to byte[] SID
                IntPtr pSid = this.groupSidList[this.currentGroup].pSid;
                ...
                if ( null == this.groupSidList[this.currentGroup].sidIssuerName )
                {
                    string name = this.groupSidList[this.currentGroup].name;
                    // Create a Principal object to represent it
                    GroupPrincipal g = GroupPrincipal.MakeGroup( this.userStoreCtx.OwningContext );
                ...
                if (sidType == SidType.FakeObject)
                {
                    ...
                    return this.userStoreCtx.ConstructFakePrincipalFromSID(sid);
                }
                ...
                if (sidType == SidType.RealObjectFakeDomain)
                {
                    ...
                    sidIssuerName = this.flatUserAuthority;
                }
                else
                {                    
                    ...
                    bool success = UnsafeNativeMethods.EqualDomainSid(this.psUserSid.DangerousGetHandle(), pSid, ref sameDomain);
                    ...
                    if (sameDomain)
                    {
                        GlobalDebug.WriteLineIf(GlobalDebug.Info, "AuthZSet", "CurrentAsPrincipal: same domain as user ({0})", this.flatUserAuthority);                    
                        sidIssuerName = this.flatUserAuthority;
                    }
                }
                ...
                {
                    sidIssuerName = this.groupSidList[this.currentGroup].sidIssuerName;   
                    GlobalDebug.WriteLineIf(GlobalDebug.Info, "AuthZSet", "CurrentAsPrincipal: different domain ({0}) than user ({1})", sidIssuerName, this.flatUserAuthority);                    
                }
                ...
                {
                    Debug.Assert(this.userType == ContextType.Domain);
                    ...
                    if (UnsafeNativeMethods.EqualDomainSid(this.psMachineSid.DangerousGetHandle(), pSid, ref inMachineDomain))
                        if (inMachineDomain)
                        {
                            ...
                            if ( !localMachineIsDC.HasValue )
                            {
                                localMachineIsDC = (bool?)Utils.IsMachineDC(null);
                                GlobalDebug.WriteLineIf(GlobalDebug.Info, "AuthZSet", "CurrentAsPrincipal: IsLocalMachine a DC, localMachineIsDC={0}", localMachineIsDC.Value);                                                                               
                            }

                            isLocalGroup = !localMachineIsDC.Value;
                        }
    	...
    	override internal bool MoveNext()
    	{
            ...
            do
            {
                ...
                {
                    GlobalDebug.WriteLineIf(GlobalDebug.Info, "AuthZSet", "MoveNext: ran off end of list ({0})", this.groupSidList.Length);                                                                
                    return false;
                }
                ...
                if (this.userType == ContextType.Machine)
                {
                    ...
                    bool sameDomain = false;
                    if (Utils.ClassifySID(pSid) == SidType.RealObject && UnsafeNativeMethods.EqualDomainSid(this.psUserSid.DangerousGetHandle(), pSid, ref sameDomain))
                    {
                        if (sameDomain)
                        {
...
#pragma warning restore 618
        sealed class SafeMemoryPtr : SafeHandle {
