...
#pragma warning restore 618
    internal abstract class SafeNCryptHandle : SafeHandle
    {
        ...
        private SafeNCryptHandle Holder
        {
            get
            {
                Contract.Requires((_ownershipState == OwnershipState.Duplicate && _holder != null) ||
                                  (_ownershipState != OwnershipState.Duplicate && _holder == null));
                Contract.Requires(_holder == null || _holder._ownershipState == OwnershipState.Holder);
            ...
            set
            {
                Contract.Ensures(_holder._ownershipState == OwnershipState.Holder);
                Contract.Ensures(_ownershipState == OwnershipState.Duplicate);
#if DEBUG
                Contract.Ensures(IsValidOpenState);
                Contract.Assert(value.IsValidOpenState);
#endif
                Contract.Assert(_ownershipState != OwnershipState.Duplicate);
                Contract.Assert(value._ownershipState == OwnershipState.Holder);
        ...
        internal T Duplicate<T>() where T : SafeNCryptHandle, new()
        {
            ...
            Contract.Ensures(_ownershipState == OwnershipState.Duplicate);
            Contract.Ensures(Contract.Result<T>()._ownershipState == OwnershipState.Duplicate);
#if DEBUG
...
#endif
            Contract.Assert(_ownershipState != OwnershipState.Holder);
        ...
        private T DuplicateDuplicatedHandle<T>() where T : SafeNCryptHandle, new()
        {
            Contract.Ensures(_ownershipState == OwnershipState.Duplicate);
            Contract.Ensures(Contract.Result<T>() != null &&
                             Contract.Result<T>()._ownershipState == OwnershipState.Duplicate);
#if DEBUG
            Contract.Ensures(IsValidOpenState);
            Contract.Ensures(Contract.Result<T>().IsValidOpenState);
        ...
        private T DuplicateOwnerHandle<T>() where T : SafeNCryptHandle, new()
        {
            ...
            Contract.Assert(IsValidOpenState);
#endif
            Contract.Assert(_ownershipState == OwnershipState.Owner);
            Contract.Assert(typeof(T) == this.GetType());
