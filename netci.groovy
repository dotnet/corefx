        ...
        ['Debug', 'Release'].each { configurationGroup ->
            ...
            def newTestJob = job(Utilities.getFullJobName(project, newTestJobName, isPR)) {
            	steps {
	                ...
	                // Run the tests
	                batchFile("run-test.cmd ${WORKSPACE}\\bin\\tests\\Windows_NT.AnyCPU.${configurationGroup} ${WORKSPACE}\\packages")
                    // Run the tests
                    batchFile("run-test.cmd ${WORKSPACE}\\bin\\tests\\AnyOS.AnyCPU.${configurationGroup} ${WORKSPACE}\\packages")
            	}
