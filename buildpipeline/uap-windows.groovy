...
simpleNode('Windows_NT','latest') {
    ...
    stage ('Sync') {
        bat '.\\sync.cmd -p -- /p:TargetGroup=uap /p:ArchGroup=x64 /p:RuntimeOS=win10'
    }
    stage ('Generate Version Assets') {
        bat '.\\build-managed.cmd -GenerateVersion'
    }
    stage ('Build Product') {
        bat ".\\build.cmd -framework:uap -buildArch=x64 -${params.Config} -- /p:SignType=real /p:RuntimeOS=win10"
    }
    stage ('Build Tests') {
        def additionalArgs = ''
        if (params.OuterLoop) {
        ...
        bat ".\\build-tests.cmd -framework:uap -buildArch=x64 -${params.Config} -SkipTests ${additionalArgs} -- /p:RuntimeOS=win10 /p:ArchiveTests=true"
    }
...
stage ('Execute Tests') {
    def contextBase
    if (params.OuterLoop) {
        contextBase = "Win x64 UAP F5 tests w/outer - ${params.Config}"
    }
    else {
        contextBase = "Win x64 UAP F5 tests - ${params.Config}"
    }
