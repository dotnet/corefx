...
The rule of thumb is that we (**corefx**) review every API that is being added to the `System` namespace. In some cases, we also review APIs that are added to namespaces, such as `Microsoft`. We mostly do this for high impact APIs, such as Roslyn, and when both the owner of the technology and us feel there is win-win for both sides if we review the APIs. However, we can't scale to review all APIs being added to .NET.
