...
We are striving to get annotations correct the "first time" and are doing due-diligence in an attempt to do so.  However, we acknowledge that we are likely to need to change some annotations in the future:

- **Mistakes.** Given the sheer number of APIs to be reviewed and annotated, we are likely to make some errors in annotations, and we'd like to be able to fix them.
- **Breadth.** We are unlikely to have the time to annotate all of the enormous number of APIs in .NET for an initial release.
- **Feedback.** We may need to revisit some "gray area" decisions as to whether a parameter or return type should be nullable or non-nullable.

As such, while changing the annotations on APIs may impact warnings in the codebase of a developer that has opted-in to nullability warnings, for at least the foreseeable future we may still do so.  We will be very thoughtful about when and how we do so.
...
- If the method checks for null and throws an ArgumentException if null is passed in for that parameter (whether explicitly in that same method or implicitly as part of some method it calls), such that there’s no way null could be passed in and have the method return successfully, the parameter should be declared as non-nullable.
- If the method fails to check for null but instead will always end up dereferencing the null and throwing a NullReferenceException, the parameter should be declared as non-nullable.
- If the parameter is explicitly documented to accept null, it should be declared nullable.
- If the parameter explicitly checks for null and does something other than through, it should be declared nullable.  This may include normalizing the input, e.g. treating null as string.Empty.
- If the parameter is optional and has a default value of null, it should be declared nullable.
...
- If under any circumstance the method might return null, assuming that’s by design, the method should be annotated to return a nullable.
- Otherwise, it should be annotated to return a non-nullable.

Annotating one of our return types as non-nullable is equivalent to documenting a guarantee that it will never return null.  Violations of that guarantee are similarly bugs to be fixed in the implementation. However, there is a huge gap here, in the form of overridable members…
...
A code review for enabling nullability generally involves two or three passes:

1.	Review all implementation changes made in the code.  Except when explicitly fixing a bug (which should be rare), the annotations employed for nullability should have zero impact on the generated IL (other than potentially some added attributes in the metadata).  The most common changes are:
a.	Adding `?` to reference type parameters and local symbols.  These inform the compiler that nulls are allowed.  For locals, they evaporate entirely at compile time.  For parameters, they impact the [Nullable(…)] attributes emitted into the metadata by the compiler, but have no effect on the implementation IL.
b.	Adding `!` to reference type usage.  These essentially suppress the null warning, telling the compiler to treat the expression as if it’s non-null.  These evaporate at compile-time.
c.	Adding `Debug.Assert(reference != null);` statements.  These inform the compiler that the mentioned reference is non-null, which will cause the compiler to factor that in and have the effect of suppressing subsequent warnings on that reference (until the flow analysis suggests that could change).  As with any Debug.Assert, these evaporate at compile-time in release builds (where DEBUG isn’t defined).
...
3.	Review *all other exported APIs* (e.g. public and protected on public types) for all reference types in both return and parameter positions.  Anything that wasn’t changed to be `?` is now defined as non-nullable.  For parameters, that means consuming code will now get a harsh warning if it tries to pass null, and thus these should be changed if null actually is allowed / expected.  For returns, it means the API will never return null; if it might return null in some circumstance, the API should be changed to return `?`.  This is the most time consuming and tedious part of the review.
